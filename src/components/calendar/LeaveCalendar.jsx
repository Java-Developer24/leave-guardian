    ...
    <div className="glass-card overflow-hidden">
      ...
      <div className="relative px-7 pt-7 pb-5">
        ...
        <div className="flex items-center justify-between">
          <button
            onClick={handlePrev}
            className="w-10 h-10 rounded-xl bg-secondary/40 hover:bg-secondary/70 flex items-center justify-center transition-all hover:scale-105 active:scale-95 border border-border/20"
            aria-label="Previous month"
          >
            <ChevronLeft size={18} />
          </button>
          <div className="text-center">
            <h3 className="text-xl font-black tracking-heading font-heading">
              {monthLabel}
            </h3>
            <div className="flex items-center justify-center gap-4 mt-2">
              <span className="flex items-center gap-1.5 text-[10px] text-muted-foreground/60">
                <Sun size={10} className="text-accent" /> {holidayCount}{" "}
                holidays
              </span>
              <span className="flex items-center gap-1.5 text-[10px] text-muted-foreground/60">
                <CalendarDays size={10} className="text-destructive/60" />{" "}
      ...
      <div className="grid grid-cols-7 gap-0 px-5 mb-1">
        {WEEKDAYS.map((d, i) => (
          <div
            key={i}
            className={`text-center text-[10px] font-bold tracking-wider uppercase py-2.5 font-heading ${i === 0 || i === 6 ? "text-primary/40" : "text-muted-foreground/40"}`}
          >
      ...
      <AnimatePresence mode="wait" initial={false}>
        ...
        >
          ...
          {days.map((day) => {
            ...
            let cellClass =
              "hover:bg-secondary/60 cursor-pointer text-foreground/80 hover:border-primary/20 border border-transparent";
            if (isSelected)
              cellClass =
                "bg-gradient-to-br from-primary to-primary/80 text-primary-foreground ring-2 ring-primary/30 shadow-lg shadow-primary/25 border border-primary/40";
            else if (isApproved)
              cellClass =
                "bg-success/10 text-success border border-success/25 shadow-sm shadow-success/5";
            else if (isRequested)
              cellClass = "bg-info/10 text-info border border-info/25";
            else if (isHoliday)
              cellClass = "bg-accent/8 text-accent border border-accent/20";
            else if (isBlocked)
              cellClass =
                "bg-slate-200/80 text-slate-500 cursor-not-allowed border border-slate-300/70 grayscale";
              ...
              >
                ...
                {isToday && !isSelected && (
                  <span
                    ...
                    style={{
                      boxShadow:
                        "0 0 12px hsla(354,100%,64%,0.3), inset 0 0 12px hsla(354,100%,64%,0.1)",
                    }}
                  />
                )}
                {isToday && (
                  <span
                    ...
                    style={{ boxShadow: "0 0 8px hsla(354,100%,64%,0.7)" }}
                  />
                )}

                <span
                  className={`font-bold text-[13px] ${isWeekend && !isSelected && !isHoliday && !isApproved && !isRequested && !isBlocked ? "text-muted-foreground/40" : ""} ${isBlocked ? "line-through decoration-slate-400/80" : ""}`}
                >
                  {day.getDate()}
                </span>
                ...
                {(isApproved || isRequested) && !isSelected && (
                  <div
                    className={`w-1.5 h-1.5 rounded-full mt-0.5 ${isApproved ? "bg-success" : "bg-info"}`}
                  />
                )}
        ...
        <div className="px-7 pb-6 pt-2 border-t border-border/15">
          <div className="flex flex-wrap gap-x-5 gap-y-2 text-[10px]">
            {[
              ...
              {
                label: "Selected",
                cls: "bg-gradient-to-r from-primary to-primary/80",
              },
              {
                label: "Approved",
                cls: "bg-success/15 border border-success/25",
              },
              { label: "Requested", cls: "bg-info/15 border border-info/25" },
              { label: "Holiday", cls: "bg-accent/10 border border-accent/20" },
              {
                label: "Blocked",
                cls: "bg-slate-200/80 border border-slate-300/70",
              },
            ].map((l) => (
              <span
                key={l.label}

...
const navSections = {
  agent: [
    ...
    {
      label: "Actions",
      items: [{ to: "/agent/leave", label: "Apply Leave", icon: Calendar }],
    },
  ],
  supervisor: [
    {
      label: "Overview",
      items: [{ to: "/supervisor/home", label: "Dashboard", icon: Home }],
    },
    {
      label: "Management",
      items: [
  ...
  admin: [
    {
      ...
      items: [
        {
          to: "/admin/uploads/schedule",
          label: "Upload Schedule",
          icon: Upload,
        },
        {
          to: "/admin/uploads/attendance",
          label: "Upload Attendance",
          icon: Upload,
        },
        {
          to: "/admin/weekoff-swaps",
          label: "Week-Off Approvals",
          icon: CheckSquare,
        },
      ],
    },
...
function getPageTitle(pathname) {
  const allSections = [
    ...(navSections.agent ?? []),
    ...(navSections.supervisor ?? []),
    ...(navSections.manager ?? []),
    ...(navSections.admin ?? []),
  ];
  return (
    allSections.flatMap((s) => s.items).find((i) => i.to === pathname)?.label ??
    "Dashboard"
  );
}
...
function UserAvatar({ name, size = "md" }) {
  ...
  return (
    <div
      className={`${sizes[size]} rounded-xl bg-primary/10 flex items-center justify-center font-bold text-primary border border-primary/15`}
    >
      {initials}
    </div>
  );
...
export default function AppShell({ children }) {
  ...
  const renderSidebarContent = ({ collapsed, mobile = false }) => (
    <div className="flex flex-col h-full">
      ...
      <div className={currentUser.role === "admin" ? "p-3 pb-2" : "p-5 pb-4"}>
        ...
        >
          <div
            className={
              currentUser.role === "admin"
                ? "w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0 border border-primary/15"
                : "w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center flex-shrink-0 border border-primary/15"
            }
          >
            <span
              className={
                currentUser.role === "admin"
                  ? "text-primary font-black text-base font-heading"
                  : "text-primary font-black text-lg font-heading"
              }
            >
              L
            </span>
          ...
          {!collapsed && (
            <div>
              <h1
                className={
                  currentUser.role === "admin"
                    ? "text-sm font-extrabold text-foreground tracking-heading font-heading"
                    : "text-base font-extrabold text-foreground tracking-heading font-heading"
                }
              >
                LSM
              </h1>
              <p
                className={
                  currentUser.role === "admin"
                    ? "text-[7px] text-muted-foreground tracking-[0.12em] uppercase font-heading mt-0.5"
                    : "text-[8px] text-muted-foreground tracking-[0.15em] uppercase font-heading mt-0.5"
                }
              >
                Leave & Shrinkage Manager Tool
              </p>
            </div>
          )}
        </div>
      ...
      <nav
        className={
          currentUser.role === "admin"
            ? "flex-1 px-2 overflow-y-auto scrollbar-hidden space-y-3 mt-1"
            : "flex-1 px-3 overflow-y-auto scrollbar-hidden space-y-5 mt-2"
        }
          ...
          <div key={section.label}>
            {!collapsed && (
              <span
                className={
                  currentUser.role === "admin"
                    ? "text-[8px] font-semibold tracking-[0.12em] uppercase text-muted-foreground/50 px-2 mb-1 block font-heading"
                    : "text-[9px] font-semibold tracking-[0.15em] uppercase text-muted-foreground/50 px-3 mb-2 block font-heading"
                }
              >
                {section.label}
              </span>
            ...
            <div
              className={
                currentUser.role === "admin" ? "space-y-0.5" : "space-y-0.5"
              }
            >
  ...
  ${
    active
                      ...
                      <motion.div
                        ...
                        className={
                          currentUser.role === "admin"
                            ...
                            : "absolute left-0 top-2 bottom-2 w-[3px] rounded-full bg-primary"
                        }
                        transition={{
                          type: "spring",
                          stiffness: 500,
      ...
      {/* Bottom */}
      <div
        className={currentUser.role === "admin" ? "p-2 mt-auto" : "p-3 mt-auto"}
      >
          ...
          {!collapsed && (
            <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-muted/30 border border-border">
              ...
              <div className="min-w-0 flex-1">
                ...
                <span
                  className={`text-[9px] px-2 py-0.5 rounded-full font-bold border mt-0.5 inline-block ${roleBadgeColors[currentUser.role]}`}
                >
        ...
        <header className="flex items-center justify-between px-5 lg:px-8 h-[60px] border-b border-border bg-background flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            >
              <ChevronLeft
                size={18}
                className={`transition-transform duration-200 ${desktopCollapsed ? "rotate-180" : ""}`}
              />
            </button>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              className="relative p-2 rounded-lg hover:bg-muted/30 transition-colors text-muted-foreground"
                ...
                {userMenuOpen && (
                  ...
                  >
                    <div className="px-3 py-3 border-b border-border mb-1">
                      ...
                      <span
                        className={`text-[9px] px-2 py-0.5 rounded-full font-bold border mt-1.5 inline-block ${roleBadgeColors[currentUser.role]}`}
                      >
                        {roleLabels[currentUser.role]}
                      </span>
                    </div>

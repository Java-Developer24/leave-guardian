...
export default function StatusChip({ status, className = "" }) {
  const color =
    LEAVE_STATUS_COLORS[status] ?? "bg-muted/40 text-muted-foreground";
  const label = LEAVE_STATUS_LABELS[status] ?? status;
  const dot = dotColors[status] ?? "bg-muted-foreground";
  const glow = glowColors[status] ?? "";
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold tracking-wide border border-current/10 ${color} ${glow} ${className}`}
    >

...
function MiniSparkline({ data, accent }) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        {icon && (
          <div
            className={`w-11 h-11 rounded-xl ${iconBgMap[accent]} flex items-center justify-center`}
          >
        ...
        {trend && (
          <span
            className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold border ${trendColors[trend.direction]}`}
          >
      ...
      <div className="flex items-end justify-between gap-2">
        <div>
          <span className="text-2xl font-extrabold tracking-heading text-foreground font-heading">
            {value}
          </span>
          {subtitle && (
      ...
      <span className="text-[10px] tracking-label uppercase text-muted-foreground font-medium font-heading">
        {label}
      </span>
    </motion.div>
  );

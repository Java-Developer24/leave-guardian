...
> = {
  supervisor: {
    ...
    highlight: "Planner",
    description:
      "Review current and next month schedules, track leave coverage, and manage week-off planning for your reporting guides.",
  },
  manager: {
...
function getRecurringWeekoffPattern(dates: string[]) {
  ...
  const grouped = dates.reduce<
    Record<number, { count: number; sampleDate: string }>
  >((acc, date) => {
...
function formatRecurringWeekoff(
  pattern: ReturnType<typeof getRecurringWeekoffPattern>,
) {
...
export default function WeeklyScheduleWorkspace({
  mode,
  initialMonthDate,
}: {
  mode: SchedulePersona;
  ...
  const monthPresets = useMemo(() => {
    let currentMonthValue = new Date(today.getFullYear(), today.getMonth(), 1);
    let nextMonthValue = new Date(today.getFullYear(), today.getMonth() + 1, 1);

    // If initialMonthDate is provided, parse it and use it as the current month
    if (initialMonthDate) {
      const [year, month] = initialMonthDate.split("-").map(Number);
      currentMonthValue = new Date(year, month - 1, 1);
      nextMonthValue = new Date(year, month, 1);
    }

    return [
      { id: "current", label: "Current Month", value: currentMonthValue },
      { id: "next", label: "Next Month", value: nextMonthValue },
    ];
  }, [today, initialMonthDate]);
  const defaultDepartmentId =
    currentUser?.departmentId ?? departments[0]?.id ?? "all";

  const [monthView, setMonthView] = useState<"current" | "next">("current");
  const [selectedWeekStart, setSelectedWeekStart] = useState(() =>
    toDateStr(startOfWeek(today)),
  );
  ...
  const [requestDetailView, setRequestDetailView] = useState<{
    kind: "history" | "queue";
    requestId: string;
  } | null>(null);
  ...
  const visibleMonth =
    monthPresets.find((item) => item.id === monthView)?.value ??
    monthPresets[0].value;
  const visibleMonthKey = getMonthKey(visibleMonth);
  const weekOptions = useMemo(
    () => getWeeksForMonth(visibleMonth),
    [visibleMonth],
  );
  ...
  const weekDays = useMemo(
    () =>
      Array.from({ length: 7 }, (_, index) =>
        addDays(activeWeekStartDate, index),
      ),
    [activeWeekStartDate],
  );
  ...
  const supervisorsByDept = useMemo(() => {
    return departments.reduce<Record<string, typeof users>>(
      (acc, department) => {
        acc[department.id] = users
          .filter(
            (user) =>
              user.role === "supervisor" && user.departmentId === department.id,
          )
  ...
  const teamAssignments = useMemo(() => {
    ...
    departments.forEach((department) => {
      ...
      const departmentAgents = users
        .filter(
          (user) =>
            user.role === "agent" && user.departmentId === department.id,
        )
        .sort((a, b) => a.name.localeCompare(b.name));
      ...
      if (departmentSupervisors.length === 1) {
        assignments[departmentSupervisors[0].id] = departmentAgents.map(
          (agent) => agent.id,
        );
        return;
      }
      ...
      departmentAgents.forEach((agent, index) => {
        const supervisor =
          departmentSupervisors[index % departmentSupervisors.length];
  ...
  const teamOptions = useMemo(() => {
    ...
    return (supervisorsByDept[scopeDepartmentId] ?? []).sort((a, b) =>
  ...
  useEffect(() => {
    ...
    if (
      selectedTeamId !== "all" &&
      !teamOptions.some((team) => team.id === selectedTeamId)
    ) {
      setSelectedTeamId("all");
    }
  ...
  const filteredAgentIds = useMemo(() => {
    if (mode === "supervisor") {
      return (
        teamAssignments[currentUser?.id ?? ""] ??
        users
          .filter(
            (user) =>
              user.role === "agent" && user.departmentId === scopeDepartmentId,
          )
          .map((user) => user.id)
      );
    }
    ...
    if (selectedTeamId !== "all") {
      ...
      const teamLead = users.find(
        (user) => user.id === selectedTeamId && user.role === "supervisor",
      );
      return users
        .filter(
          (user) =>
            user.role === "agent" &&
            user.departmentId === teamLead?.departmentId,
        )
        .map((user) => user.id);
    }
  ...
  const filteredAgentIdSet = useMemo(
    () => new Set(filteredAgentIds),
    [filteredAgentIds],
  );
  ...
  const visibleAgents = useMemo(() => {
    return users
      .filter(
        (user) => user.role === "agent" && filteredAgentIdSet.has(user.id),
      )
      .sort((a, b) => a.name.localeCompare(b.name));
  ...
  const visibleSupervisors = useMemo(() => {
    ...
    return users.filter(
      (user) =>
        user.role === "supervisor" && user.departmentId === scopeDepartmentId,
    );
  ...
  const weekRows = useMemo(() => {
    return visibleAgents.map((agent) => {
      const dayRows = weekDays.map((day) => {
        ...
        return (
          schedule.find(
            (item) => item.userId === agent.id && item.date === date,
          ) ?? null
        );
      });
      ...
      return {
        ...
        dayRows,
        weekLeaves,
        weekOffDates: dayRows
          .filter(Boolean)
          .filter((item) => item?.weekOff)
          .map((item) => item!.date),
      };
    });
  }, [
  ...
  const scopedWeekoffRequests = useMemo(() => {
    return weekoffSwapRequests
      .filter((request) => {
        ...
        return (
          filteredAgentIdSet.has(request.sourceGuideId) ||
          filteredAgentIdSet.has(request.peerGuideId)
        );
      })
      .sort((a, b) =>
        (b.history[0]?.at ?? "").localeCompare(a.history[0]?.at ?? ""),
      );
  }, [weekoffSwapRequests, filteredAgentIdSet, mode, scopeDepartmentId]);

  const selectedGuideRow =
    mode === "supervisor" || mode === "manager"
      ? (weekRows.find((row) => row.agent.id === activeGuideId) ?? null)
      : null;
  const peerGuideChoices = visibleAgents.filter(
    (agent) => agent.id !== activeGuideId,
  );
  const peerGuideRow =
    weekRows.find((row) => row.agent.id === peerGuideId) ?? null;
  ...
  const selectedGuideMonthRows = useMemo(() => {
    ...
    return schedule
      .filter(
        (row) =>
          row.userId === activeGuideId && row.date.startsWith(visibleMonthKey),
      )
      .sort((a, b) => a.date.localeCompare(b.date));
  ...
  const peerGuideMonthRows = useMemo(() => {
    ...
    return schedule
      .filter(
        (row) =>
          row.userId === peerGuideId && row.date.startsWith(visibleMonthKey),
      )
      .sort((a, b) => a.date.localeCompare(b.date));
  ...
  const historyYearOptions = useMemo(
    () =>
      Array.from(
        new Set(
          [
            ...schedule.map((row) => row.date.slice(0, 4)),
            ...weekoffSwapRequests.map((request) =>
              request.sourceDate.slice(0, 4),
            ),
          ].map(Number),
        ),
      ).sort((a, b) => a - b),
    [schedule, weekoffSwapRequests],
  );
  const guideHistoryRows = useMemo(() => {
    return approvedWeekoffRequests
      .filter(
        (request) =>
          (request.monthKey ?? request.sourceDate.slice(0, 7)) ===
            historyMonthKey &&
          (!historyGuideId ||
            request.sourceGuideId === historyGuideId ||
            request.peerGuideId === historyGuideId),
      )
      .sort((a, b) =>
        (b.history[b.history.length - 1]?.at ?? "").localeCompare(
          a.history[a.history.length - 1]?.at ?? "",
        ),
      );
  }, [approvedWeekoffRequests, historyMonthKey, historyGuideId]);
  const pendingWeekoffQueue = useMemo(
    () =>
      scopedWeekoffRequests.filter(
        (request) => request.status === "PendingAdmin",
      ),
    [scopedWeekoffRequests],
  );
  ...
  const visibleHistoryRows =
    guideHistoryRows.length > 0 ? guideHistoryRows : historyFallbackRows;
  ...
  const selectedGuideWeekMoveOption = useMemo(() => {
    ...
    return (
      selectedGuideWeekMoveOptions.find(
        (option) => option.sourceDate === sourceDate,
      ) ?? null
    );
  ...
  const selectedGuideWeekMoveTargets = useMemo(() => {
    ...
    return selectedGuideWeekMoveOptions.filter(
      (option) => option.sourceDate === sourceDate,
    );
  }, [swapMode, selectedGuideWeekMoveOptions, sourceDate]);
  ...
  const detailRequest = useMemo(() => {
    ...
    const sourceRows =
      requestDetailView.kind === "queue"
        ? pendingWeekoffQueue
        : visibleHistoryRows;
    return (
      sourceRows.find(
        (request) => request.id === requestDetailView.requestId,
      ) ??
      scopedWeekoffRequests.find(
        (request) => request.id === requestDetailView.requestId,
      ) ??
      null
    );
  ...
  useEffect(() => {
    ...
    if (swapScope === "Month") {
      setSourceDate(
        selectedGuideRecurringWeekoff?.sampleDate ??
          selectedGuideMonthOffDates[0] ??
          "",
      );
      return;
    }
  ...
  useEffect(() => {
    if (swapScope === "Month") {
      setPeerDate(
        peerGuideRecurringWeekoff?.sampleDate ??
          peerGuideMonthOffDates[0] ??
          "",
      );
      return;
    }
  ...
  const weekOffCount = weekRows.reduce(
    (count, row) => count + row.weekOffDates.length,
    0,
  );
    ...
    if (swapMode === "WeekMove" && !selectedWeekMoveValidation?.allowed) {
      showToast(
        selectedWeekMoveValidation?.reason ??
          "Select a valid prior-day move in the same week",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      showToast(
        error instanceof Error
          ? error.message
          : "Failed to submit week-off swap request",
        "error",
      );
    } finally {
      setSubmitting(false);
  ...
  const renderWeekoffRequestDetail = (
    ...
    return (
      <div className="space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <div className="mt-1 text-[11px] text-muted-foreground">
              {request.peerGuideId
                ? `Matched with ${getUserName(request.peerGuideId)}`
                : "Single-guide week-off movement"}
            </div>
          </div>
          <span
        ...
        <div className="grid grid-cols-2 gap-3 text-xs lg:grid-cols-4">
          <div className="rounded-xl border border-border bg-background/80 p-3">
            ...
            <div className="mt-1 font-semibold">
              {getWeekoffModeLabel(request)}
            </div>
          </div>
          <div className="rounded-xl border border-border bg-background/80 p-3">
            <div className="text-muted-foreground">Scope</div>
            <div className="mt-1 font-semibold">
              {getWeekoffScopeLabel(request)}
            </div>
          </div>
          <div className="rounded-xl border border-border bg-background/80 p-3">
            <div className="text-muted-foreground">
              {isHistory ? "Requested On" : "Submitted On"}
            </div>
            <div className="mt-1 font-semibold">
              {formatDate(request.history[0]?.at ?? request.sourceDate)}
            </div>
          </div>
          <div className="rounded-xl border border-border bg-background/80 p-3">
            <div className="text-muted-foreground">
              {isHistory ? "Approved On" : "Requested By"}
            </div>
            <div className="mt-1 font-semibold">
              {isHistory
                ? formatDate(
                    request.history[request.history.length - 1]?.at ??
                      request.sourceDate,
                  )
        ...
        <div
          className={`grid grid-cols-1 gap-3 ${isHistory ? "" : "lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]"}`}
        >
          ...
          {isHistory ? (
            <div className="rounded-xl border border-border bg-background/80 p-3 text-xs">
              ...
              <div className="mt-2 text-muted-foreground">
                Applied tag: {getWeekoffModeLabel(request)} in{" "}
                {getWeekoffScopeLabel(request)}.
              </div>
            </div>
          ) : (
            <div className="rounded-xl border border-warning/20 bg-warning/10 p-3 text-xs">
              <div className="text-[10px] uppercase tracking-[0.16em] text-warning/70 font-heading">
                Admin Review Focus
              </div>
              <div className="mt-2 font-semibold">
                Validate coverage for{" "}
                {request.periodType === "Month"
                  ? "the repeating month pattern"
                  : "the selected week window"}{" "}
              ...
              <div className="flex gap-2">
                {monthPresets.map((option) => (
                  <button
                    key={option.id}
                    onClick={() =>
                      setMonthView(option.id as "current" | "next")
                    }
                    className={`rounded-xl px-4 py-2 text-xs font-bold transition-colors ${
                      monthView === option.id
                        ? "bg-primary text-primary-foreground"
  ...
  <div className="flex flex-col sm:flex-row items-stretch gap-4">
    ...
    <div className="relative w-full sm:w-[260px]">
      <button
        ...
        className="w-full h-[52px] flex items-center justify-between rounded-xl border border-border bg-card px-4 text-sm font-medium hover:bg-muted/30 transition"
      >
      {new Date(selectedDate).toLocaleDateString('en-US', {
          month: 'long',
          year: 'numeric',
        })}
        {dateRangeOpen ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
      </button>

      {dateRangeOpen && (
        <div className="absolute top-full left-0 mt-2 w-full max-h-[240px] overflow-y-auto rounded-xl border border-border bg-card shadow-lg z-50">
            ...
            {last12Months.map((month) => (
              <button
                ...
                className={`w-full text-left px-3 py-2 rounded-md text-sm transition ${
                  new Date(selectedDate).toISOString() === new Date(month.value).toISOString()
                    ? 'bg-primary text-primary-foreground'
                    : 'hover:bg-muted/30'
                }`}
              >
    ...
    >
      ...
      <span >Download Attendence</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] gap-3 items-end">
              <div>
                <label className="block text-[10px] tracking-section uppercase text-muted-foreground mb-1.5 font-semibold">
                  Department
                </label>
                <select
          ...
          <div className="bg-card border border-border rounded-xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="rounded-xl border border-border bg-muted/20 p-3">
                <div className="text-muted-foreground">Guides Scheduled</div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-border flex items-center justify-between gap-3">
          ...
          <div className="text-xs text-muted-foreground text-right">
            {visibleAgents.length}{" "}
            {mode === "supervisor" ? "reporting guides" : "guides in scope"}
          </div>
        </div>
              ...
              <tbody>
                {weekRows.map((row) => {
                  const showDepartmentLabel =
                    mode !== "supervisor" && scopeDepartmentId === "all";
                    ...
                    <tr key={row.agent.id}>
                      ...
                      {row.dayRows.map((dayRow, index) => {
                        const cellDate = weekDays[index]
                          ? toDateStr(weekDays[index])
                          : "";
                        const dayLeave =
                          row.weekLeaves.find(
                            (leave) => leave.date === cellDate,
                          ) ?? null;
                        const weekoffTag = approvedWeekoffRequests
                          ...
                          .find(Boolean);
                        const isTransferredLeave =
                          dayLeave?.type === "Transfer" &&
                          dayLeave.peerId === row.agent.id;

                        return (
                          <td
                            key={`${row.agent.id}-${cellDate || index}`}
                            ...
                            {!dayRow ? (
                              ...
                              <div className="rounded-xl border border-info/20 bg-info/10 px-2.5 py-2 text-[10px]">
                                ...
                                <div className="mt-1 text-info/80">
                                  {isTransferredLeave
                                    ? `From ${getUserName(dayLeave.requesterId)}`
                                    : formatLeaveStatusLabel(dayLeave.status)}
                                </div>
                              </div>
                            ) : dayRow.weekOff ? (
                              <div
                        ...
                        <div className="flex flex-wrap gap-1">
                          {row.weekLeaves.length === 0 ? (
                            ...
                            row.weekLeaves.slice(0, 3).map((leave) => (
                              <span
                                key={leave.id}
                                className="inline-flex rounded-full border border-info/15 bg-info/10 px-2.5 py-1 text-[10px] font-semibold text-info"
                              >
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-5 py-4">
            <div>
              <div className="flex items-center gap-2 text-sm font-bold font-heading">
                <ArrowLeftRight size={15} className="text-primary" /> Week-Off
                Swap Queue
              </div>
              <div className="text-[11px] text-muted-foreground">
                ...
                <tbody>
                  {pendingWeekoffQueue.map((request) => (
                    <tr key={`queue-row-${request.id}`}>
                      ...
                      <td>
                        {formatDate(
                          request.history[0]?.at ?? request.sourceDate,
                        )}
                      </td>
      ...
      {mode === "supervisor" && (
        <div className="mt-6 rounded-xl border border-border bg-card overflow-hidden">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-5 py-4">
            <div>
              <div className="text-sm font-bold font-heading">
                Week-Off Change History
              </div>
              <div className="text-[11px] text-muted-foreground">
                Review month swaps, week moves, and week swaps for each guide
                across different months.
              </div>
            </div>
            <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
              <select
                value={historyGuideId}
                onChange={(event) => setHistoryGuideId(event.target.value)}
                className="glass-input text-sm"
              >
                <option value="">All guides</option>
                {visibleAgents.map((agent) => (
                  <option key={`history-guide-${agent.id}`} value={agent.id}>
                    {agent.name}
                  </option>
                ))}
              </select>
              ...
              >
                {historyYearOptions.map((year) => (
                  <option key={`history-year-${year}`} value={year}>
                    {year}
                  </option>
                ))}
              </select>
          ...
          {visibleHistoryRows.length === 0 ? (
            <div className="px-5 py-10 text-center text-sm text-muted-foreground">
              No approved week-off changes found for this month and guide
              selection.
            </div>
          ) : (
            <div className="space-y-4">
              {historyFallbackRows.length > 0 ? (
                <div className="mx-5 mt-5 rounded-xl border border-info/20 bg-info/10 px-4 py-3 text-xs text-info">
                  No approved records were found for the exact month filter, so
                  the latest approved week-off examples are shown below.
                </div>
              ) : null}
              <div className="overflow-x-auto">
                  ...
                  <tbody>
                    {visibleHistoryRows.map((request) => (
                      <tr key={`history-row-${request.id}`}>
                        ...
                        <td className="max-w-[240px] text-[11px] text-muted-foreground">
                          {getWeekoffResultSummary(request, getUserName)}
                        </td>
                        <td>
                          <button
            ...
            <div className="space-y-5">
                  ...
                  {[
                    {
                      id: "Week",
                      label: "Week",
                      description: "Use a single week for a move or a swap.",
                    },
                    {
                      id: "Month",
                      label: "Month",
                      description:
                        "Swap the full month week-off pattern with another guide.",
                    },
                  ].map((option) => (
                    <button
                      key={option.id}
                ...
                <div>
                  ...
                  {swapMode === "WeekMove" ? (
                    <div className="mt-2 rounded-xl border border-info/20 bg-info/10 px-4 py-3 text-xs text-info">
                      Pick any visible week first, then choose the current week
                      off on the left and the valid same-week prior day on the
                      right.
                    </div>
                  ) : null}
                </div>
                    ...
                    ? [
                        {
                          ...
                          label: "Swap week off",
                          description:
                            "Swap the entire month week-off pattern with another guide.",
                        },
                      ]
                    : [
                        {
                          ...
                          label: "Move week off",
                          description:
                            "Move one guide week off within the selected week.",
                        },
                        {
                          id: "WeekSwap",
                          label: "Swap week off",
                          description:
                            "Swap one week off between two specific guides in the selected week.",
                        },
                      ]
                  ).map((option) => (
                    <button
                      ...
                      onClick={() => {
                        setSwapMode(
                          option.id as "MonthSwap" | "WeekMove" | "WeekSwap",
                        );
              ...
              {swapMode === "WeekMove" && (
                <div className="rounded-xl border border-info/20 bg-info/10 px-4 py-3 text-xs text-info">
                  Validation note: this move stays on the same guide, only
                  shifts to within the same week, and still checks coverage plus
                  the 7-day login/work rule before it can be submitted.
                </div>
              )}
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  {swapScope === "Month" ? (
                    !selectedGuideRecurringWeekoff ? (
                      ...
                      >
                        ...
                        <div className="mt-1 opacity-80">
                          {selectedGuideRecurringWeekoff.count} week-off
                          occurrence
                          {selectedGuideRecurringWeekoff.count === 1 ? "" : "s"}{" "}
                          in {formatMonthYear(visibleMonth)}
                        </div>
                      </button>
                    )
                ...
                <div className="space-y-3">
                  ...
                  {swapMode === "WeekMove" ? (
                    !sourceDate ? (
                      <div className="rounded-xl border border-border bg-muted/20 px-4 py-5 text-center text-xs text-muted-foreground">
                        Select a current week-off day to view the available day
                        for that week.
                      </div>
                    ) : selectedGuideWeekMoveTargets.length === 0 ? (
                      <div className="rounded-xl border border-border bg-muted/20 px-4 py-5 text-center text-xs text-muted-foreground">
                        {selectedGuideWeekMoveOption?.reason ??
                          "No available prior-day move was found for this week-off selection."}
                      </div>
                    ) : (
                      selectedGuideWeekMoveTargets.map((option) => (
                        ...
                        >
                          <div className="font-semibold">
                            {formatWeekdayWithDate(option.date)}
                          </div>
                          <div className="mt-1 text-[11px] opacity-80">
                            {option.allowed
                              ? "Available day in the selected week"
                              : (option.reason ??
                                "This move is not available for the selected week-off day.")}
                          </div>
                        </button>
                      ))
                    )
                    ...
                    !peerGuideId ? (
                      <div className="rounded-xl border border-border bg-muted/20 px-4 py-5 text-center text-xs text-muted-foreground">
                        Select another guide to view the regular week-off
                        pattern.
                      </div>
                    ) : !peerGuideRecurringWeekoff ? (
                      <div className="rounded-xl border border-border bg-muted/20 px-4 py-5 text-center text-xs text-muted-foreground">
                        No regular week-off pattern available for this
                        selection.
                      </div>
                    ) : (
                      <button
                        type="button"
                        onClick={() =>
                          setPeerDate(peerGuideRecurringWeekoff.sampleDate)
                        }
                        className={`w-full rounded-xl border px-3 py-3 text-left text-xs transition-colors ${
                      ...
                      >
                        ...
                        <div className="mt-1 opacity-80">
                          {peerGuideRecurringWeekoff.count} week-off occurrence
                          {peerGuideRecurringWeekoff.count === 1 ? "" : "s"} in{" "}
              ...
              <div className="rounded-xl border border-border bg-background/80 p-4">
                ...
                <div className="mt-2 text-xs text-muted-foreground">
                  Admin approval is still required before the updated week-off
                  pattern appears in the published schedule.
                </div>
                {swapMode === "WeekMove" &&
                selectedWeekMoveValidation &&
                !selectedWeekMoveValidation.allowed ? (
              ...
              <div className="rounded-xl border border-info/20 bg-info/10 px-4 py-3 text-xs text-info">
                This request goes to admin approval first. The team dashboard
                updates only after admin approval.
              </div>
      ...
      {mode === "supervisor" && (
        <Modal
          ...
          title={
            requestDetailView?.kind === "queue"
              ? "Week-Off Swap Queue Details"
              : "Week-Off Change History Details"
          }
        >
          {detailRequest && requestDetailView
            ? renderWeekoffRequestDetail(detailRequest, requestDetailView.kind)
            : null}
        </Modal>
      )}
    </motion.div>

...
function formatWeekdayWithDate(date) {
  return `${getWeekdayName(date)} • ${formatDate(date)}`;
}

function getRecurringWeekoffPattern(dates) {
...
function formatRecurringWeekoff(pattern) {
...
function formatExactShortDate(date) {
...
export default function WeeklyScheduleWorkspace({ mode, initialMonthDate }) {
  ...
  const [swapScope, setSwapScope] = useState("Week");
  const [swapMode, setSwapMode] = useState("WeekSwap");
  ...
  const [requestDetailView, setRequestDetailView] = useState(null);
  ...
  const renderWeekoffRequestDetail = (request, kind) => {
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex gap-2">
              {monthPresets.map((option) => (
                <button
                  key={option.id}
                  onClick={() => setMonthView(option.id)}
                        ...
                        {last12Months.map((month) => (
                          <button
                            ...
                            className={`w-full text-left px-3 py-2 rounded-md text-sm transition ${
                              ...
                              new Date(month.value).toISOString()
                                ? "bg-primary text-primary-foreground"
                                : "hover:bg-muted/30"
                            }`}
                          >
                    ...
                    !selectedGuideRecurringWeekoff ? (
                      ...
                      >
                        ...
                        <div className="mt-1 opacity-80">
                          ...
                          occurrence
                          {selectedGuideRecurringWeekoff.count === 1
                            ? ""
                            : "s"}{" "}
                    ...
                    !peerGuideId ? (
                      ...
                      >
                        ...
                        <div className="mt-1 opacity-80">
                          ...
                          {peerGuideRecurringWeekoff.count === 1
                            ? ""
                            : "s"} in {formatMonthYear(visibleMonth)}
                        </div>
                      </button>
                    )
                  ) : !peerGuideRow ? (

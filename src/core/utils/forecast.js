...
export function estimateForecastVolume(date) {
...
export function estimateRequiredGuides(date) {
...
export function countScheduledGuidesForDepartment(
  ...
  const deptUserIds = new Set(
    users
      .filter(
        (user) => user.role === "agent" && user.departmentId === departmentId,
      )
      .map((user) => user.id),
  );
...
export function countActiveLeavesForDepartment(date, departmentId, leaves) {
...
export function buildLeaveSubmissionPreview(params) {
  ...
  const activeLeaves = countActiveLeavesForDepartment(
    date,
    departmentId,
    leaves,
  );
  ...
  const availableGuidesAfterApproval = Math.max(
    0,
    scheduledGuides - activeLeaves - addedLeaves,
  );

  const shrinkageBefore =
    scheduledGuides === 0 ? 0 : (activeLeaves / scheduledGuides) * 100;
  const shrinkageAfter =
    scheduledGuides === 0
      ? 0
      : ((activeLeaves + addedLeaves) / scheduledGuides) * 100;

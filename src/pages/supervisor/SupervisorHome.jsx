...
export default function SupervisorHome() {
  ...
  const [approveReviewId, setApproveReviewId] = useState(null);
  ...
  const teamAgents = useMemo(
    () =>
      users.filter(
        (user) => user.role === "agent" && user.departmentId === deptId,
      ),
    [users, deptId],
  );
  ...
  const monthOptions = useMemo(() => {
    const keys = new Set([currentMonthKey, nextMonthKey]);
  ...
  const selectedMonthDate = useMemo(
    () => new Date(`${selectedMonthKey}-01T00:00:00`),
    [selectedMonthKey],
  );
  ...
  const selectedMonthApprovedLeaves = deptLeaves.filter(
    (leave) =>
      leave.date.startsWith(selectedMonthKey) && leave.status === "Approved",
  );
  ...
  const selectedMonthForecastAlerts = useMemo(
    () =>
      openForecastAlerts.filter((alert) =>
        alert.date.startsWith(selectedMonthKey),
      ),
    [openForecastAlerts, selectedMonthKey],
  );
  const selectedMonthLeaveDays = selectedMonthApprovedLeaves.reduce(
    (total, leave) => total + leave.days,
    0,
  );
  const selectedMonthTargetHours = teamSize * standardTargetHoursPerGuide;
  const selectedMonthAchievedHours = Math.max(
    0,
    selectedMonthTargetHours - selectedMonthLeaveDays * 8,
  );
  const selectedMonthDeficitHours = Math.max(
    0,
    selectedMonthTargetHours - selectedMonthAchievedHours,
  );
  ...
  const teamSummary = useMemo(() => {
    return teamAgents
      .map((agent) => {
        ...
        const activeMonthLeaves = monthLeaves.filter(
          (leave) => !["Rejected", "Cancelled"].includes(leave.status),
        );
        const approvedMonthLeaves = monthLeaves.filter(
          (leave) => leave.status === "Approved",
        );
        const planned = activeMonthLeaves.filter(
          (leave) => leave.type === "Planned",
        ).length;
        const unplanned = activeMonthLeaves.filter(
          (leave) => leave.type === "Unplanned",
        ).length;
        const approvedDays = approvedMonthLeaves.reduce(
          (total, leave) => total + leave.days,
          0,
        );
        const approvedLeaveCount = approvedMonthLeaves.length;
        const pendingCount = activeMonthLeaves.filter((leave) =>
    ...
    selectedMonthKey,
    standardMandays,
    standardTargetHoursPerGuide,
  ]);

  const reviewLeave =
    pending.find((leave) => leave.id === approveReviewId) ?? null;
  const confirmApproveLeave =
    pending.find((leave) => leave.id === confirmApproveId) ?? null;
      ...
      <div className="mb-6 flex flex-wrap items-center gap-4 rounded-xl border border-border bg-card px-5 py-4">
        <div className="mr-auto">
          ...
          <div className="text-[11px] text-muted-foreground">
            Transfer requests stay supervisor-only, while week-off approvals
            continue to route through admin review.
          </div>
        </div>
      ...
      <div className="mb-6 rounded-xl border border-border bg-card p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <div className="mt-1 text-[11px] text-muted-foreground">
              Guides with forecast-backed leave impact in {selectedMonthLabel}{" "}
              are surfaced here before approval.
            </div>
          </div>
          <Link
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 xl:grid-cols-3">
          {selectedMonthForecastAlerts.slice(0, 3).map((alert) => (
            <div
              key={alert.id}
              className="rounded-xl border border-warning/20 bg-warning/10 px-4 py-3 text-xs"
            >
              <div className="font-semibold">
                {getUserName(alert.requesterId)}
              </div>
              <div className="mt-1 text-warning/90">
                {formatDate(alert.date)} • {alert.forecastVolume} forecast
                volume • {alert.availableGuides}/{alert.requiredGuides} guides
                after approval
              </div>
            </div>
          ))}
        ...
        <div className="mb-4 flex items-start justify-between gap-4">
          <h2 className="flex items-center gap-2 text-base font-bold font-heading">
            ...
            <span className="ml-2 text-xs font-normal text-muted-foreground">
              {pending.length} requests need your action in {selectedMonthLabel}
            </span>
          </h2>
          <Link
            to="/supervisor/approvals"
            ...
            {pendingColumns.map((column, columnIndex) => (
              <div key={`pending-column-${columnIndex}`} className="space-y-4">
                {column.map((leave, index) => {
                  ...
                  const approvalTimer = submittedEvent
                    ? getApprovalCountdown(submittedEvent.at, 72, liveNow)
                    : null;
                  const alert = openForecastAlertByLeaveId[leave.id];
                  const dayShrinkage = calcDailyShrinkage(
                    leave.date,
                    deptLeaves,
                    schedule,
                  );
                  const potentialShrinkage =
                    alert?.shrinkageAfter ?? dayShrinkage;
                    ...
                    >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-[10px] text-muted-foreground">
                              {formatDate(leave.date)} • {leave.type}
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-3">
                        ...
                        <div className="col-span-2 rounded-lg border border-border bg-muted/30 p-2 sm:col-span-1">
                          ...
                          <span
                            className={`font-semibold ${approvalTimer?.overdue ? "text-destructive" : "text-warning"}`}
                          >
                      ...
                      {(leave.type === "Swap" || leave.type === "Transfer") && (
                        <div
                          className={`rounded-xl border p-3 ${leave.type === "Swap" ? "border-info/15 bg-info/6" : "border-primary/15 bg-primary/6"}`}
                        >
                      ...
                      >
                        ...
                        <span>
                          Current shrinkage:{" "}
                          <strong>{dayShrinkage.toFixed(1)}%</strong>
                        </span>
                        <span className="mx-1">•</span>
                        <span>
                          After approval:{" "}
                          <strong>{potentialShrinkage.toFixed(1)}%</strong>
                        </span>
                      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-card">
        <div className="flex items-center justify-between border-b border-border px-5 py-3.5">
          <div>
            ...
            <div className="mt-1 text-[11px] text-muted-foreground">
              Sorted by highest production deficit and leave demand for{" "}
              {selectedMonthLabel}.
            </div>
          </div>
          <Link
      ...
      <Modal
        ...
        title={
          reviewLeave?.type === "Swap"
            ? "Review Swap Approval"
            : reviewLeave?.type === "Transfer"
              ? "Review Transfer Approval"
              : "Review Leave Approval"
        }
      >
        {reviewLeave && (
          <div className="space-y-5">
              ...
              <div className="space-y-3 rounded-xl border border-info/15 bg-info/6 p-4">
                ...
                <div className="grid grid-cols-1 gap-3 text-xs sm:grid-cols-2">
                  ...
                  <div className="rounded-xl border border-border bg-background/80 p-3">
                    ...
                    <div className="mt-1 text-sm font-semibold">
                      {reviewLeave.peerId
                        ? getUserName(reviewLeave.peerId)
                        : "Not assigned"}
                    </div>
                  </div>
                  <div className="rounded-xl border border-border bg-background/80 p-3">
                    ...
                    <div className="mt-1 text-sm font-semibold">
                      {formatDate(
                        (
                          getHistoryEntry(reviewLeave, "Submitted") ??
                          reviewLeave.history[0]
                        ).at,
                      )}
                    </div>
                  </div>
                  ...
                  <div className="rounded-xl border border-border bg-background/80 p-3">
                    ...
                    <div className="mt-1 text-sm font-semibold">
                      {reviewLeave.type === "Swap"
                        ? reviewLeave.peerLeaveDate
                          ? formatDate(reviewLeave.peerLeaveDate)
                          : "Not provided"
                        : "Supervisor initiated transfer"}
                    </div>
                  </div>
            ...
            <div className="rounded-xl border border-border bg-background/80 p-4">
              ...
              <div className="mt-2 text-sm text-muted-foreground">
                {reviewNotes[reviewLeave.id]?.trim() ||
                  "No approval note added yet."}
              </div>
            </div>

            <div className="flex justify-end gap-3">
              <button
                onClick={closeApproveFlow}
                className="rounded-xl border border-border px-5 py-2.5 text-sm font-semibold transition-colors hover:bg-muted/30"
              >
                Cancel
              </button>
              <button
                onClick={() => setConfirmApproveId(reviewLeave.id)}
        ...
        {confirmApproveLeave && (
          <div className="space-y-5">
            ...
            <div className="rounded-xl border border-border bg-muted/20 p-4 text-sm">
              <div className="font-semibold">
                {getUserName(confirmApproveLeave.requesterId)}
              </div>

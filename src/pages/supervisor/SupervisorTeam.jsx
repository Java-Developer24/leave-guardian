...
export default function SupervisorTeam() {
  ...
  const currentMonthYearLabel = today.toLocaleDateString("en-US", {
    month: "long",
    year: "numeric",
  });
  ...
  const nextMonthYearLabel = nextMonthDate.toLocaleDateString("en-US", {
    month: "long",
    year: "numeric",
  });
  ...
  const teamMembers = useMemo(() => {
    const agents = users.filter(
      (u) => u.role === "agent" && u.departmentId === deptId,
    );
    return agents
      .map((agent) => {
        const agentLeaves = leaves.filter((l) => l.requesterId === agent.id);
        const currentMonthLeaves = agentLeaves.filter((l) =>
          l.date.startsWith(currentMonthKey),
        );
        const currentMonthApproved = currentMonthLeaves.filter(
          (l) => l.status === "Approved",
        );
        const approved = currentMonthApproved.length;
        const approvedDays = currentMonthApproved.reduce(
          (total, leave) => total + leave.days,
          0,
        );
        const pending = currentMonthLeaves.filter((l) =>
          ["PendingSupervisor", "PendingPeer", "Submitted"].includes(l.status),
        ).length;
        const rejected = agentLeaves.filter(
          (l) => l.status === "Rejected",
        ).length;
        ...
        const nextMonthPending = agentLeaves.filter(
          (leave) =>
            leave.date.startsWith(nextMonthKey) &&
            ["PendingSupervisor", "PendingPeer", "Submitted"].includes(
              leave.status,
            ),
        ).length;
        const totalCurrentMonthRequests = currentMonthLeaves.filter(
  ...
  const getRiskLevel = (deficit) => {
    ...
    if (deficit >= 1)
      return { label: "At Risk", color: "bg-info/10 text-info border-info/15" };
    return {
      label: "On Track",
      color: "bg-success/10 text-success border-success/15",
    };
  };
    ...
    <motion.div {...pageTransition}>
      ...
      <div className="glass-card-featured px-6 py-4 mb-6 flex flex-wrap items-center gap-8">
        {[
          ...
          {
            value: teamMembers.filter((m) => m.deficit === 0).length,
            label: "On track for production",
            color: "text-success",
            icon: CheckCircle,
          },
          {
            value: teamMembers.filter((m) => m.deficit >= 1 && m.deficit <= 16)
              .length,
            label: "At risk for production",
            color: "text-warning",
            icon: AlertTriangle,
          },
          {
            value: teamMembers.reduce((s, m) => s + m.approved, 0),
            label: `Leaves taken in current ${currentMonthYearLabel}`,
            color: "text-foreground",
            icon: TrendingUp,
          },
          {
            value: teamMembers.reduce((s, m) => s + m.nextMonthPending, 0),
            label: `Pending leave requests for ${nextMonthYearLabel}`,
            color: "text-info",
            icon: Clock,
          },
        ].map((s) => (
          <div key={s.label} className="flex items-center gap-3">
            ...
            <div>
              <div className={`text-xl font-extrabold font-heading ${s.color}`}>
                {s.value}
              </div>
              <div className="text-[9px] tracking-section uppercase text-muted-foreground/50 font-heading">
                {s.label}
              </div>
            </div>
          </div>
      ...
      <div className="mb-5">
        <div className="relative max-w-xs">
          <Search
            size={14}
            className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground/30 pointer-events-none"
          />
          <input
            type="text"
            value={searchTerm}
        ...
        {filtered.map((m) => {
          ...
          return (
            <motion.div
              ...
              {...cardHover}
              className="glass-card gradient-border p-5 space-y-4"
            >
              {/* Header */}
              <div className="flex items-start justify-between">
                ...
                <span
                  className={`text-[9px] px-2.5 py-1 rounded-full font-bold border ${risk.color}`}
                >
                  {risk.label}
                </span>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  {
                    v: m.approved,
                    l: `${currentMonthLabel} Leaves`,
                    c: "text-foreground",
                  },
                  {
                    v: m.nextMonthPending,
                    l: `${nextMonthLabel} Leaves`,
                    c: "text-warning",
                  },
                  ...
                  <div
                    key={s.l}
                    className="text-center p-2 bg-card/50 rounded-lg border border-border/10"
                  >
                    <div className={`text-base font-bold font-heading ${s.c}`}>
                      {s.v}
                    </div>
                    <div className="text-[7px] text-muted-foreground/50 tracking-wider uppercase font-heading">
                      {s.l}
                    </div>
                  </div>
                ))}
              ...
              {m.deficit >= 1 && (
                <div
                  className={`flex items-center gap-2 text-[10px] px-3 py-2 rounded-xl border ${
                    m.deficit > 16

...
function ShrinkageGauge({ now, after, cap }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="flex items-center gap-2.5 text-[10px]">
        ...
        <span
          className={`w-10 text-right font-bold ${exceedsCap ? "text-destructive" : "text-success"}`}
        >
          {after}%
        </span>
      </div>
...
export default function SupervisorApprovals() {
  ...
  const [forecastReviewed, setForecastReviewed] = useState({});
  ...
  const pending = useMemo(
    () =>
      leaves
        .filter(
          (leave) =>
            leave.departmentId === deptId &&
            leave.status === "PendingSupervisor",
        )
        .sort((a, b) =>
          (getHistoryEntry(b, "Submitted")?.at ?? b.date).localeCompare(
            getHistoryEntry(a, "Submitted")?.at ?? a.date,
          ),
        ),
    [leaves, deptId],
  );
  ...
  const openForecastAlerts = useMemo(
    () =>
      forecastAlerts
        .filter(
          (alert) => alert.departmentId === deptId && alert.status === "Open",
        )
  ...
  const alertByLeaveId = useMemo(
    () =>
      openForecastAlerts.reduce((acc, alert) => {
  ...
  const reviewLeave =
    pending.find((leave) => leave.id === reviewAction?.id) ?? null;
  const reviewAlert = reviewLeave ? alertByLeaveId[reviewLeave.id] : undefined;
  const requiresForecastReview =
    reviewAction?.action === "approve" && !!reviewAlert;
  const hasReviewedForecast = reviewLeave
    ? Boolean(forecastReviewed[reviewLeave.id])
    : false;
  const noteRequired =
    reviewAction?.action === "approve" && requiresForecastReview;
  ...
  const getShrinkageChange = (leave) => {
    ...
    const departmentActiveLeaves = deptLeaves
      .filter(
        (item) =>
    ...
    const now =
      scheduledGuides === 0
        ? 0
        : (departmentActiveLeaves / scheduledGuides) * 100;
    ...
    return {
      ...
      exceedsCap: after > cap,
      cap,
    };
  };

  const getApprovalTimer = (leave) => {
    const submittedAt = (
      getHistoryEntry(leave, "Submitted") ?? leave.history[0]
    )?.at;
    return submittedAt ? getApprovalCountdown(submittedAt, 72, liveNow) : null;
  };

  const getTransferAvailability = (userId, monthKey) => {
    return leaves
      .filter(
        (leave) =>
  ...
  const getTransferGuideSnapshot = (userId, monthKey) => {
    ...
    return {
      approved: monthLeaves.filter((leave) => leave.status === "Approved")
        .length,
      pending: monthLeaves.filter((leave) =>
        ["PendingSupervisor", "PendingPeer", "Submitted"].includes(
          leave.status,
        ),
      ).length,
      planned: monthLeaves.filter((leave) => leave.type === "Planned").length,
      unplanned: monthLeaves.filter((leave) => leave.type === "Unplanned")
        .length,
  ...
  const transferGuideAvailability = useMemo(
    () =>
      transferGuideId
        ? getTransferAvailability(transferGuideId, transferMonthKey)
        : [],
  ...
  const handleCreateTransfer = async () => {
    if (
      ...
      !transferGuideId ||
      !transferBuddyId ||
      !selectedTransferGuideLeave
    ) {
      showToast(
        "Select guide and their leave date, plus transfer buddy",
        "error",
      );
      return;
    }
    ...
    try {
      const created = await apiService.createLeave({
        ...
        reason:
          ...
          `Supervisor transfer request for ${selectedTransferGuideLeave.date}`,
        status: "Approved",
      ...
      setActiveTab("queue");
      showToast(
        `Transfer approved! ${getUserName(transferGuideId)}'s leave on ${selectedTransferGuideLeave.date} has been transferred to ${getUserName(transferBuddyId)}. Admins have been notified.`,
        "success",
      );
    } catch {
    ...
    <motion.div {...pageTransition}>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-6 rounded-xl border border-border bg-card px-6 py-3.5">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <span className="block text-lg font-black font-heading">
              {pending.length}
            </span>
      ...
      >
        <TabsList className="mb-6 border border-border bg-muted/50">
          <TabsTrigger
            value="queue"
            className="text-xs font-semibold data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
          >
            Approval Queue
          </TabsTrigger>
          <TabsTrigger
            ...
            <div className="space-y-4">
              {queueCards.map((leave, index) => {
                ...
                const submittedEvent =
                  getHistoryEntry(leave, "Submitted") ?? leave.history[0];
                const acceptedByPeerEvent = getHistoryEntry(
                  leave,
                  "Accepted by Peer",
                );
                ...
                const transferAvailability =
                  leave.type === "Transfer"
                    ? {
                        requester: getTransferAvailability(
                          leave.requesterId,
                          requestMonthKey,
                        ),
                        buddy: leave.peerId
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="mt-1 text-[11px] text-muted-foreground">
                            {formatDate(leave.date)} • {leave.type} • submitted{" "}
                            {submittedEvent
                              ? formatDate(submittedEvent.at)
                              : "—"}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-3 text-xs md:grid-cols-4">
                          ...
                          <div className="rounded-xl border border-border bg-muted/20 p-3">
                            ...
                            <div
                              className={`mt-1 font-semibold ${timer?.overdue ? "text-destructive" : "text-warning"}`}
                            >
                        ...
                        {(leave.type === "Swap" ||
                          leave.type === "Transfer") && (
                          <div
                            className={`rounded-xl border p-4 ${leave.type === "Swap" ? "border-info/15 bg-info/6" : "border-primary/15 bg-primary/6"}`}
                          >
                            ...
                            <div className="mt-3 grid grid-cols-1 gap-3 text-xs sm:grid-cols-2 xl:grid-cols-3">
                              ...
                              <div className="rounded-xl border border-border bg-background/80 p-3">
                                ...
                                <div className="mt-1 font-semibold">
                                  {acceptedByPeerEvent
                                    ? formatDate(acceptedByPeerEvent.at)
                              ...
                              <div className="rounded-xl border border-border bg-background/80 p-3">
                                ...
                                <div className="mt-1 font-semibold">
                                  {leave.type === "Swap"
                                    ? leave.peerLeaveDate
                                      ? formatDate(leave.peerLeaveDate)
                                      : "Not provided"
                        ...
                        {transferAvailability && (
                          <div className="rounded-xl border border-border bg-background/80 p-4">
                            <div className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground/60 font-heading">
                              Available Leaves In{" "}
                              {formatMonthYear(requestMonthKey)}
                            </div>
                            <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
                              <div className="rounded-xl border border-border bg-muted/20 p-3 text-xs">
                                ...
                                <div className="mt-2 flex flex-wrap gap-2">
                                  {transferAvailability.requester.length > 0 ? (
                                    transferAvailability.requester.map(
                                      (item) => (
                                        <span
                                          key={item.id}
                                          className="rounded-full border border-primary/15 bg-primary/8 px-2.5 py-1 font-semibold text-primary"
                                        >
                                    ...
                                    <span className="text-muted-foreground">
                                      No active leave availability in this
                                      month.
                                    </span>
                                  )}
                                </div>
                              ...
                              <div className="rounded-xl border border-border bg-muted/20 p-3 text-xs">
                                ...
                                <div className="mt-2 flex flex-wrap gap-2">
                                  {transferAvailability.buddy.length > 0 ? (
                                    transferAvailability.buddy.map((item) => (
                                      <span
                                        key={item.id}
                                        className="rounded-full border border-info/15 bg-info/8 px-2.5 py-1 font-semibold text-info"
                                      >
                                        {formatDate(item.date)}
                                      </span>
                                    ))
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            <div className="rounded-xl border border-primary/15 bg-primary/6 p-5">
              <div className="text-sm font-bold font-heading">
                Supervisor Leave Transfer
              </div>
              <div className="mt-1 text-[11px] text-muted-foreground">
                Create and approve a transfer request for two guides directly.
                Once approved, admins will be notified. Select two guides and
                their respective leave dates to initiate the transfer.
              </div>
            </div>
                ...
                <div className="rounded-xl border border-border bg-background/75 p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <div className="text-sm font-bold">
                        {transferGuideId
                          ? `${getUserName(transferGuideId)} leave inventory`
                          : "Select a guide first"}
                      </div>
                      <div className="mt-1 text-[11px] text-muted-foreground">
                        Active leave dates for{" "}
                        {formatMonthYear(transferMonthKey)}.
                      </div>
                    </div>
                      ...
                      <div className="flex flex-wrap gap-2 text-[10px]">
                        <span className="rounded-full border border-border bg-background/80 px-3 py-1.5">
                          ...
                          <span className="font-semibold">
                            {transferGuideSnapshot.approved}
                          </span>
                        </span>
                        <span className="rounded-full border border-border bg-background/80 px-3 py-1.5">
                          Pending{" "}
                          <span className="font-semibold">
                            {transferGuideSnapshot.pending}
                          </span>
                        </span>
                        <span className="rounded-full border border-border bg-background/80 px-3 py-1.5">
                          Planned{" "}
                          <span className="font-semibold">
                            {transferGuideSnapshot.planned}
                          </span>
                        </span>
                        <span className="rounded-full border border-border bg-background/80 px-3 py-1.5">
                          Unplanned{" "}
                          <span className="font-semibold">
                            {transferGuideSnapshot.unplanned}
                          </span>
                        </span>
                      </div>
                    ) : null}
                  ...
                  {transferGuideSnapshot ? (
                    <div className="mt-3 grid grid-cols-1 gap-2 text-xs sm:grid-cols-2">
                      <div className="rounded-xl border border-border bg-background/80 p-3">
                        <div className="text-muted-foreground">
                          Leave Dates In Month
                        </div>
                        <div className="mt-1 font-semibold">
                          {transferGuideSnapshot.activeDates}
                        </div>
                      </div>
                      <div className="rounded-xl border border-border bg-background/80 p-3">
                        <div className="text-muted-foreground">
                        ...
                        <div className="mt-1 font-semibold">
                          {transferGuideSnapshot.nextLeaveDate
                            ? formatDate(transferGuideSnapshot.nextLeaveDate)
                            : "No leave planned"}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="mt-4 space-y-2">
                    {transferGuideAvailability.length > 0 ? (
                      transferGuideAvailability.map((leave) => (
                        <button
                          key={leave.id}
                          type="button"
                ...
                <div className="rounded-xl border border-border bg-background/80 p-4">
                  ...
                  <div className="mt-3 space-y-3 text-xs">
                    <div className="rounded-xl border border-border bg-muted/20 p-3">
                      ...
                      <div className="mt-1 text-[11px] text-muted-foreground">
                        {selectedTransferGuideLeave
                          ? `Leave date: ${formatDate(selectedTransferGuideLeave.date)}`
                          : "Pick one leave date from the guide inventory"}
                      </div>
                    </div>
                    <div className="rounded-xl border border-border bg-muted/20 p-3">
                      <div className="text-muted-foreground">
                    ...
                    <div className="rounded-xl border border-info/20 bg-info/10 px-4 py-3 text-xs text-info">
                      ✓ The selected leave will be removed from{" "}
                      {transferGuideId
                        ? getUserName(transferGuideId)
                        : "the guide"}{" "}
        ...
        <TabsContent value="forecast">
          <div className="mb-5 rounded-xl border border-info/20 bg-info/10 px-4 py-3 text-xs text-info">
            Forecast volumes, scheduled guides, required guides, and shrinkage
            impact in this tab are calculated at the department level only.
          </div>
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-xl font-black font-heading">
                    {
                      openForecastAlerts.filter(
                        (alert) => alert.availableGuides < alert.requiredGuides,
                      ).length
                    }
                  </div>
                  <div className="text-[10px] text-muted-foreground">
                    Understaffed forecast days
                  </div>
                </div>
              </div>
            </div>
          ...
          {openForecastAlerts.length === 0 ? (
            <div className="rounded-xl border border-border bg-card p-14 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                No manager review items right now.
              </p>
            </div>
          ) : (
              ...
              {openForecastAlerts.map((alert) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="text-sm font-bold">
                        {getUserName(alert.requesterId)}
                      </div>
                      <div className="mt-1 text-[11px] text-muted-foreground">
                        {formatDate(alert.date)} • submitted{" "}
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    ...
                    <div className="rounded-xl border border-border bg-muted/20 p-3">
                      ...
                      <div
                        className={`mt-1 font-semibold ${alert.availableGuides < alert.requiredGuides ? "text-destructive" : "text-success"}`}
                      >
                        {alert.availableGuides}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="rounded-xl border border-border bg-background/80 p-4">
                    ...
                    <div className="mt-1 text-sm font-semibold">
                      {alert.shrinkageBefore}% to {alert.shrinkageAfter}%
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-1 gap-3 text-xs sm:grid-cols-2">
              <div className="rounded-xl border border-border bg-muted/20 p-3">
                ...
                <div className="mt-1 text-sm font-semibold">
                  {getUserName(reviewLeave.requesterId)}
                </div>
              </div>
              <div className="rounded-xl border border-border bg-muted/20 p-3">
            ...
            {reviewAlert && reviewAction?.action === "approve" && (
              <div className="rounded-xl border border-warning/20 bg-warning/10 px-4 py-3 text-xs text-warning">
                <div className="font-semibold">
                  Forecast alert is active for this guide.
                </div>
                <div className="mt-1">
                  The confirm button stays disabled until forecast review is
                  completed.
                </div>
              </div>
            )}
            ...
            <div>
              ...
              <textarea
                ...
                placeholder={
                  noteRequired
                    ? "Add notes after completing forecast review..."
                    : "Optional comment..."
                }
              />
            </div>
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-2 gap-3 text-xs">
              ...
              <div className="rounded-xl border border-border bg-muted/20 p-3">
                ...
                <div
                  className={`mt-1 text-lg font-black font-heading ${reviewAlert.availableGuides < reviewAlert.requiredGuides ? "text-destructive" : "text-success"}`}
                >
            ...
            <div className="rounded-xl border border-border bg-background/80 p-4">
              ...
              <div className="mt-1 text-sm font-semibold">
                {reviewAlert.shrinkageBefore}% to {reviewAlert.shrinkageAfter}%
              </div>
            </div>

            <div className="flex justify-end gap-3">
              <button
                onClick={() => setForecastReviewOpen(false)}
                className="rounded-xl border border-border px-5 py-2.5 text-sm font-semibold transition-colors hover:bg-muted/30"
              >
                Back
              </button>

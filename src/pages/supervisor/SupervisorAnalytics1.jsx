...
export default function SupervisorAnalytics() {
  ...
  const plannedLeaveCount = currentMonthLeaves
    .filter((leave) => leave.type === "Planned")
    .reduce((total, leave) => total + leave.days, 0);
  const unplannedLeaveCount = currentMonthLeaves
    .filter((leave) => leave.type === "Unplanned")
    .reduce((total, leave) => total + leave.days, 0);
  ...
  const recommendations = useMemo(() => {
    ...
    if (highRiskRows.length > 0) {
      const primarySpan = riskSpans[0] ?? {
        start: highRiskRows[0].date,
        end: highRiskRows[0].date,
        days: 1,
      };
      items.push({
        key: "forecast-risk",
    ...
    if (weekoffOpportunity) {
      ...
      items.push({
        ...
        description:
          "Week-off movement or week-off swap can still be reviewed as alternatives where they do not reduce same-day availability. Use the Team Schedule planner to validate week-off changes before approving more leave on high-risk days.",
        severity: "low",
      });
    }
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <div className="text-[11px] text-muted-foreground">
              Select a supervisor and department to view their performance
              analytics.
            </div>
          </div>
        </div>
        ...
        <div className="self-start rounded-xl border border-border bg-card p-3.5 lg:p-4">
          <div className="mb-3 flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[9px] leading-5 text-muted-foreground">
                ...
                {forecastMonthLabel}. Check {forecastNextMonthLabel} next for
                the forecasted shrinkage calculation for the entire team.
              </p>
            </div>
            ...
            {forecastCalendarDays.map((day) => {
              ...
              return (
                <div
                  ...
                  className={`min-h-[122px] rounded-lg border px-1.5 py-1.5 ${dayCardClass}`}
                >
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3 text-[10px] text-muted-foreground">
            <span className="flex items-center gap-1">
              <span className="h-2 w-2 rounded-full bg-amber-500" /> High risk
            </span>
            <span className="flex items-center gap-1">
        ...
        <div className="self-start rounded-xl border border-border bg-card p-3 ">
          <div className="mb-8">
            ...
            <p className="text-[10px] text-muted-foreground">
              Month-wise attendance view for the selected date range, including
              variance number and variance percentage.
            </p>
          </div>
                  ...
                  <div>
                    ...
                    <div className="h-2 rounded-full bg-muted/25">
                      <div
                        ...
                        style={{
                          width: `${Math.max(16, (item.planned / attendanceTrendMax) * 100)}%`,
                        }}
                      />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="h-2 rounded-full bg-muted/25">
                      <div
                        ...
                        style={{
                          width: `${Math.max(16, (item.actual / attendanceTrendMax) * 100)}%`,
                        }}
                      />
                    </div>
                  </div>
        ...
        <div className="rounded-xl border border-border bg-card p-3.5 lg:p-4">
          <div className="mb-3">
            <h3 className="flex items-center gap-2 text-sm font-bold font-heading">
              <AlertTriangle size={14} className="text-warning" /> High Risk
              Dates
            </h3>
            <p className="text-[10px] text-muted-foreground">
              Daily risk details update from the month and date filters above,
              including shrinkage, leaves taken, pending requests, and
              forecasted shrinkage.
            </p>
          </div>
          ...
          <div className="mb-4 rounded-xl border border-border bg-background/80 p-3">
            <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <div className="mt-1 text-[11px] text-muted-foreground">
                  Switch this card between a single month, a month range, or a
                  custom date range.
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  <button
                    key={option.key}
                    type="button"
                    onClick={() => setTeamRiskFilterMode(option.key)}
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                <div>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setTeamRiskMonthFrom(event.target.value)
                    }
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                <div>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setTeamRiskDateFrom(event.target.value)
                    }
                    className="glass-input text-sm"
                  />
                </div>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2 rounded-xl border border-border bg-muted/20 px-3 py-2 text-[11px]">
              ...
              <span className="text-muted-foreground">
                Active window: {teamRiskWindow.helperLabel}
              </span>
            </div>
          </div>
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-1 text-[11px] text-muted-foreground">
                        Leaves: {item.approved} Approved • Planned{" "}
                        {item.planned} • Unplanned {item.unplanned} • Total
                        Leave percentage {item.leavePct}%
                      </div>
                    </div>
                    <span
                  ...
                  <div className="mt-3">
                    <Progress
                      value={Math.min(100, item.leavePct * 4)}
                      className="h-2.5"
                    />
                  </div>
                </div>
      ...
      <div className="mb-5 rounded-xl border border-border bg-card p-3.5 lg:p-4">
        <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">
              Calendar-wise daily view of shrinkage percentage, forecast volume,
              scheduled guides, and required guides. The current day is
              highlighted, the previous day is softly marked, and high-risk
              dates are highlighted stronger.
            </p>
          </div>

...
import { AlertTriangle, CheckCircle, XCircle, Inbox, Shield, BarChart3 } from 'lucide-react';
...
export default function SupervisorApprovals() {
  ...
  return (
    <motion.div {...pageTransition}>
      ...
      {/* Summary strip */}
      <div className="glass-card px-6 py-3.5 mb-6 flex items-center gap-6">
      ...
      <div className="glass-card-featured overflow-hidden">
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-sm premium-table">
            <thead>
              <tr>{['Agent', 'Date', 'Type', 'Reason', 'Shrinkage Impact', 'Actions'].map(h => <th key={h}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {pending.map((l, i) => {
                const s = getShrinkageChange(l);
                return (
                  <motion.tr key={l.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}>
                    <td>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-semibold">{getUserName(l.requesterId)}</div>
                          <div className="text-[10px] text-muted-foreground">Agent</div>
                        </div>
                      </div>
                    </td>
                    <td className="font-semibold">{formatDate(l.date)}</td>
                    <td><span className="text-[10px] bg-secondary/60 px-2.5 py-0.5 rounded-lg font-bold uppercase tracking-wider border border-border/20">{l.type}</span></td>
                    <td className="text-muted-foreground text-xs max-w-[150px] truncate">{l.reason || '—'}</td>
                    <td className="min-w-[200px]">
                      ...
                      {s.exceedsCap && (
                        <span className="flex items-center gap-1.5 text-[9px] bg-destructive/10 text-destructive px-2.5 py-1 rounded-full font-bold mt-1.5 w-fit border border-destructive/12">
                          <AlertTriangle size={9} /> Exceeds {s.cap}% cap
                        </span>
                      )}
                    </td>
                    <td>
                      <div className="flex gap-2">
                        <button onClick={() => setCommentModal({ id: l.id, action: 'approve' })}
                          className="px-4 py-2 text-xs font-bold bg-success/8 text-success border border-success/20 rounded-xl hover:bg-success/15 hover:shadow-lg hover:shadow-success/10 transition-all flex items-center gap-1.5">
                          <CheckCircle size={12} /> Approve
                        </button>
                        <button onClick={() => setCommentModal({ id: l.id, action: 'reject' })}
                          className="px-4 py-2 text-xs font-bold bg-destructive/8 text-destructive border border-destructive/20 rounded-xl hover:bg-destructive/15 hover:shadow-lg hover:shadow-destructive/10 transition-all flex items-center gap-1.5">
            ...
            return (
              <div key={l.id} className="p-5 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center text-[10px] font-bold text-primary">{getInitials(l.requesterId)}</div>
                    <div>
                ...
                <ShrinkageGauge now={s.now} after={s.after} cap={s.cap} />
                <div className="flex gap-2">
                  <button onClick={() => setCommentModal({ id: l.id, action: 'approve' })} className="flex-1 py-2.5 text-xs font-bold bg-success/8 text-success border border-success/20 rounded-xl flex items-center justify-center gap-1.5"><CheckCircle size={12} /> Approve</button>
                  <button onClick={() => setCommentModal({ id: l.id, action: 'reject' })} className="flex-1 py-2.5 text-xs font-bold bg-destructive/8 text-destructive border border-destructive/20 rounded-xl flex items-center justify-center gap-1.5"><XCircle size={12} /> Reject</button>
                </div>
              </div>
            );
          })}
      ...
      <Modal open={!!commentModal} onClose={() => { setCommentModal(null); setComment(''); }} title={commentModal?.action === 'approve' ? 'Approve Leave' : 'Reject Leave'}>
        <div className="space-y-5">
          ...
          <textarea value={comment} onChange={e => setComment(e.target.value)} rows={3} className="glass-input resize-none text-sm" placeholder="Optional comment..." />

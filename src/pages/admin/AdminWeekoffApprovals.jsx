...
export default function AdminWeekoffApprovals() {
  ...
  const getWeekDayNames = (weekStart) => {
  ...
  const approvedLeaveRequests = useMemo(
    () =>
      leaves
        .filter(
          (leave) => leave.status === "Approved" && leave.type !== "Transfer",
        )
        .sort((a, b) => {
          const aApprovalEvent = a.history.find((h) => h.action === "Approved");
          const bApprovalEvent = b.history.find((h) => h.action === "Approved");
    ...
    <motion.div {...pageTransition}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          <div className="mt-2 text-2xl font-black font-heading">
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-border">
            <div className="text-sm font-bold font-heading">
              Pending Week-Off Swaps
            </div>
            <div className="text-[11px] text-muted-foreground">
              Only approved requests will update the published schedule.
            </div>
          </div>
          ...
          {pendingRequests.length === 0 ? (
            <div className="p-14 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                No week-off swaps waiting for review.
              </p>
            </div>
          ) : (
              ...
              {pendingRequests.map((request) => (
                <div key={request.id} className="p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                    <div className="rounded-xl border border-border bg-muted/20 p-3">
                      ...
                      <div className="mt-2 font-semibold text-sm">
                        {getWeekDayNames(request.sourceDate)
                          .slice(0, 2)
                          .join(", ")}{" "}
                        week-off
                      </div>
                    ...
                    <div className="rounded-xl border border-border bg-muted/20 p-3">
                      ...
                      <div className="mt-2 font-semibold text-sm">
                        {getWeekDayNames(request.peerDate)
                          .slice(0, 2)
                          .join(", ")}{" "}
                        week-off
                      </div>
                  ...
                  <div className="rounded-xl border border-info/15 bg-info/8 px-4 py-3 text-xs text-info">
                    ℹ️ <strong>Swap Summary:</strong>{" "}
                    {getUserName(request.sourceGuideId)}'s{" "}
                    {getWeekDayNames(request.sourceDate).slice(0, 2).join(", ")}{" "}
                    week-off is being swapped with{" "}
                    {request.peerGuideId
                      ? `${getUserName(request.peerGuideId)}'s ${getWeekDayNames(request.peerDate).slice(0, 2).join(", ")} week-off`
                      : "another guide"}
                    . Both parties will be notified of changes.
                  </div>
                 ...
                 {getWeekoffRequestDescription(request, getUserName)}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                    <div className="rounded-xl border border-success/15 bg-success/8 p-3 text-success flex items-center gap-2">
                      <CheckCircle size={14} /> 7th day working condition check:
                      Passed
                    </div>
                    <div className="rounded-xl border border-success/15 bg-success/8 p-3 text-success flex items-center gap-2">
                      <CheckCircle size={14} /> Required HC Availability check:
                      Passed
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3">
                    <button
                      onClick={() =>
                        setReviewAction({ id: request.id, action: "approve" })
                      }
                    ...
                    <button
                      onClick={() =>
                        setReviewAction({ id: request.id, action: "reject" })
                      }
        ...
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[11px] text-muted-foreground">
                Recently approved or rejected week-off swaps.
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {reviewedRequests.slice(0, 12).map((request) => {
              const approvalEvent = request.history.find(
                (h) => h.action === "Approved" || h.action === "Rejected",
              );
              const approvalDate = approvalEvent?.at
                ? new Date(approvalEvent.at).toLocaleDateString("en-US", {
                    weekday: "short",
                    month: "short",
                    day: "numeric",
                  })
              ...
              const applicationDate =
                request.history.length > 0
                  ? new Date(request.history[0].at).toLocaleDateString(
                      "en-US",
              ...
              return (
                <div
                  key={request.id}
                  className="rounded-xl border border-border bg-background/80 px-4 py-3 text-xs"
                >
                  ...
                  <div className="mt-2 flex flex-wrap gap-3 text-[10px] text-muted-foreground/80">
                    <span className="flex items-center gap-1">
                      <Clock size={11} /> Applied: {applicationDate}
                    </span>
                    <span className="flex items-center gap-1">
                      <CheckCircle size={11} /> Approved: {approvalDate}
                    </span>
                  </div>
                </div>
              );
      ...
      <Modal
        ...
        title={
          reviewAction?.action === "approve"
            ? "Approve Week-Off Swap"
            : "Reject Week-Off Swap"
        }
      >
      ...
      >
        <div className="space-y-4">
          {/* Leave Type Filter */}
          <div className="mb-4">
            <label className="block text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground mb-2">
              Filter by Type
            </label>
            <select
          ...
          approvedLeaveRequests.length === 0 ? (
            ...
            <>
              {(notificationTypeFilter === "all" ||
                notificationTypeFilter === "Transfer") &&
                transferredLeaves.length > 0 && (
                  <div className="space-y-3">
                    ...
                    {transferredLeaves.map((leave) => {
                      const approvalEvent = leave.history.find(
                        (h) => h.action === "Approved",
                      );
                      const approvalDateText =
                        "Isaac Allen approved on " +
                        (approvalEvent?.at
                          ? new Date(approvalEvent.at).toLocaleDateString(
                              "en-US",
                              {
                                weekday: "short",
                                month: "short",
                        ...
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div className="text-xs flex-1">
                              <div className="font-semibold">
                                {getUserName(leave.requesterId)}'s leave
                                transferred to{" "}
                                {leave.peerId
                              ...
                              <div className="mt-2 text-muted-foreground space-y-1 text-[10px]">
                                ...
                                <div>
                                  ...
                                  {leave.reason ||
                                    "Leave transfer by supervisor"}
                                </div>
                                <div>
                                  ✅ <strong>Approved Info:</strong>{" "}
                                  {approvalDateText}
                                </div>
                              </div>
                            </div>
              ...
              {(notificationTypeFilter === "all" ||
                notificationTypeFilter === "Planned" ||
                notificationTypeFilter === "Unplanned") &&
                approvedLeaveRequests.length > 0 && (
                  <div className="space-y-3 mt-4 pt-4 border-t border-border">
                    ...
                    {approvedLeaveRequests
                      .filter(
                        (leave) =>
                          notificationTypeFilter === "all" ||
                          leave.type === notificationTypeFilter,
                      )
                      .map((leave) => {
                        const approvalEvent = leave.history.find(
                          (h) => h.action === "Approved",
                        );
                        const approvalDateText = approvalEvent?.at
                          ? new Date(approvalEvent.at).toLocaleDateString(
                              "en-US",
                              {
                                weekday: "short",
                            ...
                            <div className="flex items-start justify-between gap-3">
                              <div className="text-xs flex-1">
                                <div className="font-semibold">
                                  ...
                                  {leave.type} Leave Request Approved
                                </div>
                                <div className="mt-2 text-muted-foreground space-y-1 text-[10px]">
                                  <div>
                                    📅 <strong>Date:</strong>{" "}
                                  ...
                                  <div>
                                    📊 <strong>Duration:</strong> {leave.days}{" "}
                                    day(s)
                                  </div>
                                  <div>
                                    💬 <strong>Reason:</strong>{" "}

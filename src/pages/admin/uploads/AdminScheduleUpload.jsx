        ...
        <div className="glass-card p-10 mb-6">
          <div className="upload-zone">
            ...
            <p className="text-sm text-muted-foreground mb-8 max-w-sm mx-auto">
              Drag & drop a CSV or JSON file here, or use the sample data
            </p>
            <button
              onClick={handleMockUpload}
              className="btn-primary-gradient text-primary-foreground px-8 py-3.5 rounded-2xl text-sm font-bold flex items-center gap-2.5 mx-auto"
            >
      ...
      {preview && (
        <div className="glass-card overflow-hidden">
          <div className="p-6 border-b border-border/20 flex items-center justify-between">
            <div>
              <h2 className="font-bold flex items-center gap-2 font-heading">
                <CheckCircle size={16} className="text-success" /> Preview —{" "}
                {preview.length} rows
              </h2>
            ...
            <div className="flex gap-2.5">
              <button
                ...
                className="px-5 py-3 text-sm bg-secondary/50 rounded-2xl font-medium hover:bg-secondary/70 transition-colors"
              >
                Discard
              </button>
              <button
                onClick={handleSave}
                disabled={saving}
                className="px-5 py-3 text-sm btn-primary-gradient text-primary-foreground rounded-2xl font-bold disabled:opacity-50"
              >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm premium-table">
              <thead>
                <tr>
                  {["User", "Date", "Shift Start", "Shift End", "Week Off"].map(
                    (h) => (
                      <th key={h}>{h}</th>
                    ),
                  )}
              ...
              <tbody>
                {preview.map((row, i) => (
                  <tr key={i}>
                    ...
                    <td>
                      <span
                        className={`text-[10px] px-2.5 py-0.5 rounded-full font-semibold border ${row.weekOff ? "bg-warning/10 text-warning border-warning/12" : "bg-success/10 text-success border-success/12"}`}
                      >
                        {row.weekOff ? "Yes" : "No"}
                      </span>
                    </td>

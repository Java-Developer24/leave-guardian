...
import { Activity, Search, Filter, Building2, XCircle,  ArrowUpRight, ArrowDownRight, Layers,  Zap,  Shield, Eye,  } from 'lucide-react';
import {
    ...
    <g>
      ...
      {width > 50 && height > 40 && (
        <>
          ...
          <text x={x + 10} y={y + 50} fontSize={10} fontWeight={600} fill={color} fontFamily="Funnel Sans">{props.payload?.shrinkage?.toFixed(1) || '0'}% shrinkage</text>
        </>
      )}
    </g>
  );
...
export default function AdminAnalytics() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'daywise'>('overview');
  ...
  const deptBreakdown = useMemo(() => {
    return departments
      ...
      .map(dept => {
        ...
        return { id: dept.id, dept: dept.name, agents, leaves: dl.length, approved: da, rejected: dr, pending: dp, unplanned: du, avgPerAgent: avg, shrinkage };
  ...
  const treemapData = useMemo(() => deptBreakdown.slice(0, 8).sort((a, b) => b.shrinkage - a.shrinkage).map(d => ({ name: d.dept, size: d.leaves, shrinkage: d.shrinkage })), [deptBreakdown]);
  ...
  const highRiskDepts = useMemo(() => deptBreakdown.filter(d => d.shrinkage >= rules.maxDailyPct), [deptBreakdown, rules.maxDailyPct]);
  const totalScheduledGuides = useMemo(() => totalAgents * 20, [totalAgents]);
  const actualHours = useMemo(() => Math.max(0, (totalScheduledGuides - approved) * 8), [totalScheduledGuides, approved]);
  const targetHours = useMemo(() => totalScheduledGuides * 8, [totalScheduledGuides]);
  const plannedLeaveCount = useMemo(() => filteredLeaves.filter(l => l.type === 'Planned').length, [filteredLeaves]);
  const unplannedLeaveCount = useMemo(() => filteredLeaves.filter(l => l.type === 'Unplanned').length, [filteredLeaves]);
  const overallShrinkagePct = useMemo(() => totalScheduledGuides > 0 ? Math.round((approved / totalScheduledGuides) * 100 * 10) / 10 : 0, [approved, totalScheduledGuides]);
  const plannedShrinkagePct = useMemo(() => totalScheduledGuides > 0 ? Math.round((plannedLeaveCount / totalScheduledGuides) * 100 * 10) / 10 : 0, [plannedLeaveCount, totalScheduledGuides]);
  const unplannedShrinkagePct = useMemo(() => totalScheduledGuides > 0 ? Math.round((unplannedLeaveCount / totalScheduledGuides) * 100 * 10) / 10 : 0, [unplannedLeaveCount, totalScheduledGuides]);
  ...
  const buildAssistantReply = (userPrompt: string) => {
    ...
    if (loweredPrompt.includes('shrinkage') || loweredPrompt.includes('forecast')) {
      const avgShrinkage = (deptBreakdown.reduce((sum, d) => sum + d.shrinkage, 0) / deptBreakdown.length).toFixed(1);
    ...
    if (loweredPrompt.includes('approval') || loweredPrompt.includes('pending')) {
 ...
 const handleSendChat = () => {
  ...
  return (
    <motion.div {...pageTransition}>
      ...
      {/* Tab Navigation */}
      <div className="mb-6">
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'overview' | 'daywise')}>
      ...
      <div className="bg-card border border-border rounded-xl p-4 mb-5 flex flex-col md:flex-row items-start md:items-center gap-3">
        ...
        <div className="flex gap-0.5 bg-muted/50 rounded-lg p-0.5 border border-border">
          {([ 'jan', 'feb', 'mar','apr'] as const).map(t => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-6">
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          <p className="text-[10px] text-muted-foreground mb-4">Compare planned vs actual attendance for selected months</p>
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={monthlyTrend} barGap={4}>
              ...
              <Bar dataKey="approved" name="Planned" fill="hsl(152, 69%, 42%)" fillOpacity={0.8} radius={[4, 4, 0, 0]} barSize={32} />
              <Bar dataKey="rejected" name="Actual" fill="hsl(215, 100%, 58%)" fillOpacity={0.8} radius={[4, 4, 0, 0]} barSize={32} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-6">
        ...
        <div className="lg:col-span-2 bg-card border border-border rounded-xl p-5">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <Treemap data={treemapData} dataKey="size" nameKey="name" content={<TreemapContent {...({} as TreemapContentProps)} />} animationDuration={400} />
          </ResponsiveContainer>
        </div>
      </div>
        ...
        <div className="overflow-x-auto max-h-[320px] overflow-y-auto scrollbar-hidden">
          <table className="w-full text-[11px] premium-table">
            <thead className="sticky top-0 z-10">
              <tr>
                <th>Department</th><th>Agents</th><th>Total Request</th><th>Unplanned</th><th>Approved</th><th>Rejected</th><th>Pending</th><th>Avg/Agent</th><th>Shrinkage</th><th>Status</th>
              </tr>
            </thead>
            <tbody>
              {deptBreakdown.map((d, i) => (
                ...
                >
                  ...
                  <td className="text-info font-semibold">{d.unplanned}</td>
            ...
            <div className="gap-4 grid grid-cols-1 lg:grid-cols-2 mb-6 mt-6">
              ...
              <div className="self-start rounded-xl border border-border bg-card p-3">
                <div className="mb-8">
                  ...
                  <p className="text-[8px] text-muted-foreground">
                    Month-wise attendance view across all departments, including variance.
                  </p>
                </div>

                <div className="grid grid-cols-3 gap-3 md:grid-cols-1">
                  {monthlyTrend.map(item => (
                    <div key={`attendance-${item.month}`} className="rounded-xl border border-border bg-background/80 p-3">
                      <div className="flex flex-wrap items-start justify-between gap-2">
                        ...
                        <div className={`rounded-full border px-2.5 py-1 text-[10px] font-bold ${
                          ...
                          {(item.rejected - item.approved) < 0 ? '-' : '+'}{Math.abs(item.rejected - item.approved)}
                        </div>
                      </div>
                      ...
                      <div className="mt-3 space-y-2.5 text-[11px]">
                        <div>
                          <div className="mb-1 flex items-center justify-between gap-2">
                            ...
                            <span className="font-semibold">{item.approved}</span>
                          </div>
                          <div className="h-2 rounded-full bg-muted/25">
                            <div
                              className="h-2 rounded-full bg-primary/75"
                              style={{ width: `${Math.max(16, (item.approved / monthlyTrend[0]?.total || 100) * 100)}%` }}
                            />
                          </div>
                        </div>
                        <div>
                          <div className="mb-1 flex items-center justify-between gap-2">
                            ...
                            <span className="font-semibold">{item.rejected}</span>
                          </div>
                          <div className="h-2 rounded-full bg-muted/25">
                            <div
                              className="h-2 rounded-full bg-success/80"
                              style={{ width: `${Math.max(16, (item.rejected / monthlyTrend[0]?.total || 100) * 100)}%` }}
                            />
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-2 text-[11px]">
                        <div className="rounded-lg border border-border bg-muted/20 p-2.5">
                          ...
                          <div className="mt-1 font-semibold">{item.rejected - item.approved}</div>
                        </div>
                        <div className="rounded-lg border border-border bg-muted/20 p-2.5">
                          <div className="text-muted-foreground">Variance %</div>
                          <div className="mt-1 font-semibold">{((((item.rejected - item.approved) / item.approved) * 100) || 0).toFixed(1)}%</div>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="self-start rounded-xl border border-border bg-card p-3.5 lg:p-4">
                <div className="mb-3">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    Daily metrics: shrinkage, forecast volume, and across-department analysis.
                  </p>
                </div>
                    ...
                    <div key={item.id} className="rounded-xl border border-border bg-muted/20 p-3.5">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          ...
                          <div className="mt-1 text-[11px] text-muted-foreground">
                            Leaves: {item.approved} Approved • {item.leaves} Total • {item.shrinkage}% Shrinkage
                          </div>
                        </div>
                        <span className={`rounded-full border px-3 py-1 text-[10px] font-bold ${
                          ...
                          {item.shrinkage > 8 ? 'High' : item.shrinkage > 5 ? 'Moderate' : 'Low'}
                        </span>
                      </div>
                      <div className="mt-3">
                        <Progress value={Math.min(100, item.shrinkage * 4)} className="h-2.5" />
                      </div>
                    </div>
                  ))}
                </div>

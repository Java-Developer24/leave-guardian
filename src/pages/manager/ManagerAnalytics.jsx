...
export default function ManagerAnalytics() {
  const { departments, users, leaves, schedule, forecastAlerts, rules } =
    useAppStore();
  ...
  const [selectedForecastAlert, setSelectedForecastAlert] = useState(null);
  const [enterpriseDateFrom, setEnterpriseDateFrom] = useState(
    toDateStr(new Date(today.getFullYear(), today.getMonth(), 1)),
  );
  const [enterpriseDateTo, setEnterpriseDateTo] = useState(
  ...
  const [performanceMonthKey, setPerformanceMonthKey] =
  ...
  const [teamRiskDateFrom, setTeamRiskDateFrom] = useState(
    toDateStr(new Date(today.getFullYear(), today.getMonth(), 1)),
  );
  const [teamRiskDateTo, setTeamRiskDateTo] = useState(
    toDateStr(new Date(today.getFullYear(), today.getMonth() + 1, 0)),
  );
  ...
  const filteredSupervisors = useMemo(() => {
    ...
    return users
      .filter(
        (u) =>
          u.role === "supervisor" && u.departmentId === selectedDepartmentId,
      )
      .sort((a, b) => a.name.localeCompare(b.name));
  ...
  useMemo(() => {
    if (
      ...
      !filteredSupervisors.find((s) => s.id === selectedSupervisorId)
    ) {
  ...
  const departmentRows = useMemo(() => {
    const deptList =
      enterpriseDeptId === "all"
        ? departments
        : departments.filter((d) => d.id === enterpriseDeptId);

    return deptList
      .map((dept) => {
        const deptGuides = users.filter(
          (user) => user.role === "agent" && user.departmentId === dept.id,
        );
        const deptSupervisors = users.filter(
          (user) => user.role === "supervisor" && user.departmentId === dept.id,
        );
        const deptLeaves = leaves.filter(
          (leave) => leave.departmentId === dept.id,
        );
        const pending = deptLeaves.filter((leave) =>
          ["PendingSupervisor", "PendingPeer"].includes(leave.status),
        ).length;
        const approved = deptLeaves.filter(
          (leave) => leave.status === "Approved",
        ).length;
        const shrinkage = calcDailyShrinkage(todayStr, deptLeaves, schedule);
        const alerts = forecastAlerts.filter(
          (alert) => alert.departmentId === dept.id && alert.status === "Open",
        ).length;
  ...
  const summaryMetrics = useMemo(() => {
    ...
    const deptGuides = users.filter(
      (u) =>
        u.role === "agent" && deptList.some((d) => d.id === u.departmentId),
    );
    const deptSupervisors = deptList.flatMap((d) =>
      users.filter((u) => u.role === "supervisor" && u.departmentId === d.id),
    );
    const deptLeaves = leaves.filter((l) =>
      deptList.some((d) => d.id === l.departmentId),
    );
    const deptAlerts = forecastAlerts.filter(
      (a) =>
        deptList.some((d) => d.id === a.departmentId) && a.status === "Open",
    );
  ...
  const supervisorGuides = useMemo(() => {
    ...
    return users.filter(
      (u) => u.role === "agent" && u.departmentId === supervisor.departmentId,
    );
  ...
  const supervisorLeaves = useMemo(() => {
    ...
    if (selectedSupervisorId === "all") {
      return leaves.filter(
        (l) =>
          deptList.some((d) => d.id === l.departmentId) &&
          supervisorGuides.some((g) => g.id === l.requesterId),
      );
    }
  ...
  const performanceCalendarDays = useMemo(
    () => getCalendarDays(performanceMonthKey),
    [performanceMonthKey],
  );
  const performanceCalendarOffset = useMemo(
    () => new Date(performanceCalendarDays[0]).getDay(),
    [performanceCalendarDays],
  );
  ...
  const dailyMetricsByDate = useMemo(() => {
    ...
    performanceCalendarDays.forEach((day) => {
      ...
      const dayScheduled = schedule.filter(
        (s) =>
          s.date === dayStr && supervisorGuides.some((g) => g.id === s.userId),
      ).length;
      ...
      const shrinkagePct =
        dayScheduled > 0
          ? Number(((activeLeaves / dayScheduled) * 100).toFixed(1))
          : 0;
      const plannedShrinkagePct =
        dayScheduled > 0
          ? Number(((dayPlannedLeaves / dayScheduled) * 100).toFixed(1))
          : 0;
      const unplannedShrinkagePct =
        dayScheduled > 0
          ? Number(((dayUnplannedLeaves / dayScheduled) * 100).toFixed(1))
          : 0;
      // Calculate forecast metrics
      const requiredGuides = Math.max(
        1,
        dayScheduled - Math.floor(dayScheduled * 0.1),
      );
      ...
      const forecastedShrinkagePct =
        dayScheduled > 0
          ? Number(
              (((activeLeaves + coverageGap) / dayScheduled) * 100).toFixed(1),
            )
          : shrinkagePct;
      const isHighRisk =
        availableGuides < requiredGuides || shrinkagePct >= maxShrinkagePct;
      metrics[dayStr] = {
        date: dayStr,
  ...
  const summaryAnalyticsMetrics = useMemo(() => {
    ...
    const monthLeaves = supervisorLeaves.filter(
      (l) => l.date >= monthStr && l.date <= monthEndStr,
    );
    const high_risk_days = Object.values(dailyMetricsByDate).filter(
      (m) => m.isHighRisk,
    ).length;
    ...
    return {
      ...
      pending: monthLeaves.filter((l) =>
        ["PendingSupervisor", "PendingPeer", "Submitted"].includes(l.status),
      ).length,
      weekoffSwaps: monthLeaves.filter(
        (l) =>
          l.type === "Swap" && ["Approved", "PendingAdmin"].includes(l.status),
      ).length,
      supervisorGuides: supervisorGuides.length,
    };
  }, [dailyMetricsByDate, supervisorLeaves, supervisorGuides, today, todayStr]);

  const handlePerformanceMonthStep = (direction) => {
  ...
  const teamRiskWindow = useMemo(() => {
    const getLabel = () => {
      if (teamRiskFilterMode === "month") {
        ...
        const from = new Date(
          teamRiskMonthFrom.split("-").map(Number)[0],
          teamRiskMonthFrom.split("-").map(Number)[1] - 1,
        ).toLocaleDateString("en-US", { month: "short", year: "numeric" });
        const to = new Date(
          teamRiskMonthTo.split("-").map(Number)[0],
          teamRiskMonthTo.split("-").map(Number)[1] - 1,
        ).toLocaleDateString("en-US", { month: "short", year: "numeric" });
        return `${from} - ${to}`;
      } else {
        const from = new Date(teamRiskDateFrom).toLocaleDateString("en-US", {
          month: "short",
          day: "numeric",
        });
        const to = new Date(teamRiskDateTo).toLocaleDateString("en-US", {
          month: "short",
          day: "numeric",
        });
        return `${from} - ${to}`;
  ...
  const shrinkageForecastData = useMemo(() => {
    ...
    return lastSixMonths.map((monthKey) => {
      ...
      const monthLeaves = leaves.filter(
        (l) =>
          l.date >= monthStart && l.date <= monthEnd && l.status === "Approved",
      );
      // Get all schedule entries for this month
      const monthSchedule = schedule.filter(
        (s) => s.date >= monthStart && s.date <= monthEnd && !s.weekOff,
      );
      ...
      const avgDailyShrinkage =
        totalScheduled > 0 ? (totalLeaves / totalScheduled) * 100 : 0;
  ...
  const monthlyAttendanceTrend = useMemo(() => {
    ...
    const rangeMonthKeys = getMonthKeysBetween(
      toDateStr(startDate),
      toDateStr(endDate),
    );
    ...
    return rangeMonthKeys.map((monthKey) => {
      ...
      const monthPlanned = schedule.filter(
        (row) =>
          ...
          row.date >= monthStartStr &&
          row.date <= monthEndStr &&
          !row.weekOff,
      ).length;
      ...
      const monthApprovedLeaves = leaves
        .filter(
          (leave) =>
            ...
            leave.date >= monthStartStr &&
            leave.date <= monthEndStr &&
  ...
  return (
    <motion.div {...pageTransition}>
      ...
      {/* ANALYTICS VIEW */}
      {analyticsTab === "analytics" && <SupervisorAnalytics />}
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            <div>
              ...
              <div className="text-[11px] text-muted-foreground">
                Filter by department and supervisor, and select a month to view
                targeted analytics.
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 items-center justify-between gap-3 mt-4">
              <div>
                <label className="block text-[10px] tracking-section text-center uppercase text-muted-foreground mb-1.5 font-semibold">
                  Month
                </label>
                <input
                  type="month"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setEnterpriseDateFrom(event.target.value)
                  }
                  className="glass-input text-sm w-full"
                />
              </div>
          ...
          >
            <motion.div variants={staggerItem}>
              <KpiCard
                label="Departments"
                value={summaryMetrics.departments}
            ...
            <motion.div variants={staggerItem}>
              <KpiCard
                label="Supervisors"
                value={summaryMetrics.supervisors}
                icon={<Users size={18} />}
                accent="accent"
              />
            </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            {departmentRows.map((row) => (
              <div
                key={row.id}
                className="bg-card border border-border rounded-xl p-5 space-y-4 hover:shadow-lg transition-shadow"
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-sm font-bold">{row.name}</div>
                    <div className="text-[10px] text-muted-foreground mt-1">
                      {row.guides} guides • {row.supervisors} supervisors
                    </div>
                  </div>
                  <span
                    className={`text-[10px] px-2.5 py-1 rounded-full font-bold border ${row.shrinkage > 8 ? "bg-destructive/10 text-destructive border-destructive/15" : row.shrinkage > 5 ? "bg-warning/10 text-warning border-warning/15" : "bg-success/10 text-success border-success/15"}`}
                  >
                    {row.shrinkage > 8
                      ? "High Risk"
                      : row.shrinkage > 5
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  ...
                  <button
                    onClick={() => {
                      const alert = forecastAlerts.find(
                        (a) => a.departmentId === row.id && a.status === "Open",
                      );
                      if (alert) {
                        setSelectedForecastAlert(alert);
                      }
                    }}
                  ...
                  >
                    ...
                    {row.alerts > 0 && (
                      <Eye size={14} className="shrink-0 mt-0.5 text-warning" />
                    )}
                  </button>
                </div>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex flex-col">
            ...
            >
              ...
              <div className="sticky top-0 z-20 border-b border-border bg-card/95 backdrop-blur p-4 flex items-center justify-between shrink-0">
                <div>
                  <h2 className="text-2xl font-bold tracking-heading">
                    Forecast Alert Details
                  </h2>
                  <p className="text-xs text-muted-foreground mt-1">
                    Complete forecast coverage and impact analysis
                  </p>
                </div>
                <button
                  onClick={() => setSelectedForecastAlert(null)}
              ...
              <div className="overflow-y-auto flex-1">
                <div className="p-4 space-y-4 max-w-md">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="rounded-xl border border-border bg-muted/20 p-3">
                      ...
                      <div className="mt-3 font-semibold text-2xl">
                        {formatDate(selectedForecastAlert.date)}
                      </div>
                    </div>
                    <div className="rounded-xl border border-border bg-muted/20 p-3">
                      <div className="text-muted-foreground text-[10px] uppercase tracking-wider font-semibold">
                      ...
                      <div className="mt-3">
                        <span className="inline-flex px-4 py-2 rounded-lg bg-warning/10 text-warning text-sm font-bold border border-warning/15">
                          {selectedForecastAlert.status}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="rounded-xl border border-info/20 bg-info/6 p-4 space-y-3">
                    <div className="text-xs font-semibold text-info uppercase tracking-wider">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                      <div className="rounded-xl border border-border bg-background/80 p-3">
                        ...
                        <div className="mt-3 text-3xl font-black font-heading text-info">
                          {selectedForecastAlert.forecastVolume}
                        </div>
                      </div>
                      <div className="rounded-xl border border-border bg-background/80 p-3">
                        <div className="text-muted-foreground text-[10px] uppercase tracking-wider font-semibold">
                        ...
                        <div className="mt-3 text-3xl font-black font-heading">
                          {selectedForecastAlert.requiredGuides}
                        </div>
                      </div>
                      <div className="rounded-xl border border-border bg-background/80 p-3">
                        <div className="text-muted-foreground text-[10px] uppercase tracking-wider font-semibold">
                        ...
                        <div className="mt-3 text-3xl font-black font-heading text-accent">
                          {selectedForecastAlert.scheduledGuides}
                        </div>
                      </div>
                      <div className="rounded-xl border border-border bg-background/80 p-3">
                        <div className="text-muted-foreground text-[10px] uppercase tracking-wider font-semibold">
                        ...
                        <div
                          className={`mt-3 text-3xl font-black font-heading ${selectedForecastAlert.availableGuides < selectedForecastAlert.requiredGuides ? "text-destructive" : "text-success"}`}
                        >
                  ...
                  <div className="rounded-xl border border-warning/20 bg-warning/6 p-4">
                    <div className="flex items-center justify-between mb-4">
                      <div className="text-xs font-semibold text-warning uppercase tracking-wider">
                        Shrinkage Impact Assessment
                      </div>
                    </div>
                    <div className="grid grid-cols-3 gap-2">
                      <div className="rounded-lg bg-background/80 p-2 text-center">
                        <div className="text-muted-foreground text-[11px] uppercase tracking-wider font-semibold">
                          Before
                        </div>
                        <div
                      ...
                      <div className="rounded-lg bg-background/80 p-2 text-center">
                        ...
                        <div
                          className={`mt-3 text-3xl font-black ${selectedForecastAlert.shrinkageAfter > 8 ? "text-destructive" : "text-warning"}`}
                        >
                    ...
                    <div className="mt-3 text-xs text-muted-foreground bg-background/40 p-2 rounded-lg">
                      {selectedForecastAlert.shrinkageAfter >
                      selectedForecastAlert.shrinkageBefore

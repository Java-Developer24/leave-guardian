...
export default function AgentHome() {
  ...
  const approved = myOwnLeaves.filter(
    (leave) => leave.status === "Approved",
  ).length;
  const pending = myOwnLeaves.filter((leave) =>
    ["PendingSupervisor", "PendingPeer", "Submitted"].includes(leave.status),
  ).length;
  const rejected = myOwnLeaves.filter(
    (leave) => leave.status === "Rejected",
  ).length;
  const leaveSwaps = myOwnLeaves.filter(
    (leave) => leave.type === "Swap",
  ).length;

  const pendingRequests = myOwnLeaves.filter((leave) =>
    ["PendingSupervisor", "PendingPeer", "Submitted"].includes(leave.status),
  );
  const pendingSupervisorCount = myOwnLeaves.filter(
    (leave) => leave.status === "PendingSupervisor",
  ).length;
  const pendingPeerCount = myOwnLeaves.filter(
    (leave) => leave.status === "PendingPeer",
  ).length;
  ...
  const currentMonthLabel = pageNow.toLocaleDateString("en-US", {
    month: "long",
    year: "numeric",
  });
  const currentMonthShort = pageNow.toLocaleDateString("en-US", {
    month: "long",
  });
  ...
  const monthlyUsed = myOwnLeaves.filter(
    (leave) =>
  ...
  const monthlyBreakdown = useMemo(() => {
    return Array.from({ length: 3 }, (_, index) => {
      const d = new Date(
        pageNow.getFullYear(),
        pageNow.getMonth() - (2 - index),
        1,
      );
      const monthKey = getMonthKey(d);
      const monthLabel = d.toLocaleDateString("en-US", { month: "short" });
      const monthLeaves = myOwnLeaves.filter((leave) =>
      ...
      leaves
        .filter(
          (leave) =>
            ...
            leave.status === "PendingPeer",
        )
        .sort((a, b) =>
          (b.history[0]?.at ?? b.date).localeCompare(
            a.history[0]?.at ?? a.date,
          ),
        ),
    [leaves, currentUser],
  );
  ...
  const selectedAction =
    incomingSwapActions.find((leave) => leave.id === selectedActionId) ?? null;
  const isSameDayIncomingSwap = Boolean(
    selectedAction &&
    selectedAction.peerLeaveDate &&
    selectedAction.date === selectedAction.peerLeaveDate,
  );
  const quickActionClass =
    "w-full rounded-xl border border-border/40 bg-card/80 px-4 py-3 text-foreground font-semibold flex items-center gap-2.5 text-xs justify-center transition-all hover:bg-primary/8 hover:border-primary/35 hover:text-primary";
  ...
  return (
    <motion.div {...pageTransition}>
      <SectionHeader
        ...
        action={
          <div className="rounded-2xl border border-border bg-card/80 px-4 py-2 text-right">
            <div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground/60 font-heading">
              Current Month
            </div>
      ...
      <motion.div
        ...
        initial="initial"
        animate="animate"
        className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5 mb-7"
      >
        <motion.div variants={staggerItem}>
          <KpiCard
            label="Total Requests"
            value={myOwnLeaves.length}
            icon={<FileText size={20} />}
            accent="primary"
            sparkline={monthlyBreakdown.map((month) => month.total)}
          />
        </motion.div>
        <motion.div variants={staggerItem}>
          <KpiCard
            ...
            accent="success"
            trend={{
              value: `${myOwnLeaves.length > 0 ? Math.round((approved / myOwnLeaves.length) * 100) : 0}%`,
              direction: "up",
            }}
          />
        </motion.div>
        ...
        <div className="lg:col-span-2 glass-card-featured">
          <div className="px-6 py-4 border-b border-border/15 flex items-center justify-between bg-gradient-to-r from-warning/3 to-transparent">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-sm font-bold tracking-heading font-heading">
                  Pending Requests
                </h2>
                <div className="flex flex-wrap items-center gap-2 mt-1">
                  ...
                  <span className="text-[10px] text-foreground bg-muted/30 px-2 py-0.5 rounded-full border border-border/20">
                    Supervisor: {pendingSupervisorCount}
                  </span>
                  <span className="text-[10px] text-foreground bg-muted/30 px-2 py-0.5 rounded-full border border-border/20">
                    Peer: {pendingPeerCount}
                  </span>
                </div>
              </div>
            </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                          <span className="text-[9px] bg-secondary/50 px-2 py-0.5 rounded-md font-medium border border-border/15">
                            {leave.type}
                          </span>
                          {leave.peerId && (
                            <span className="text-[9px] text-muted-foreground">
                              with {getUserName(leave.peerId)}
                            </span>
                          )}
        ...
        <div className="space-y-4">
              ...
              <div className="space-y-2.5 text-xs flex-1">
                {[
                  ...
                  },
                  {
                    ...
                    value: Math.max(
                      0,
                      rules.agentMonthlyLeaveCap - monthlyUsed,
                    ),
                    color: "text-success",
                    bg: "bg-success/6",
                  },
                  {
                    label: "Cap",
                    value: `${rules.agentMonthlyLeaveCap}/mo`,
                  ...
                  <div
                    key={item.label}
                    className={`flex items-center justify-between ${item.bg} rounded-lg px-3 py-1.5 border border-border/10`}
                  >
          ...
          <div className="glass-card accent-top-card p-6">
            ...
            <div className="space-y-2">
              <Link to="/agent/leave" className={quickActionClass}>
                <Send size={14} /> Apply for Leave
              </Link>
              <Link to="/agent/summary" className={quickActionClass}>
                <FileText size={14} /> View Summary
              </Link>
              <Link to="/agent/leave?tab=swap" className={quickActionClass}>
                <ArrowLeftRight size={14} /> Leave Swaps
              </Link>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 glass-card p-6">
          ...
          {recentActivity.length === 0 ? (
            <div className="py-10 text-center">
              <Sparkles
                size={28}
                className="mx-auto mb-3 text-muted-foreground/15"
              />
            ...
            <div className="relative">
              ...
              <div className="space-y-4">
                {recentActivity.map((event, index) => (
                  ...
                  >
                    <div
                      className={`w-[10px] h-[10px] rounded-full mt-2 flex-shrink-0 relative z-10 ring-3 ring-background ${event.kind === "unplanned" ? "bg-warning shadow-[0_0_8px_hsla(35,100%,60%,0.35)]" : event.status === "Approved" ? "bg-success shadow-[0_0_8px_hsla(152,69%,42%,0.4)]" : "bg-destructive shadow-[0_0_8px_hsla(0,85%,60%,0.4)]"}`}
                    />
        ...
        <div className="glass-card p-5 self-start">
          ...
          {incomingSwapActions.length === 0 ? (
            <p className="text-xs text-muted-foreground/50 text-center py-8">
              No swap requests waiting for you.
            </p>
          ) : (
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <div className="text-[10px] text-muted-foreground mt-1">
                          Requested{" "}
                          {formatDate(leave.history[0]?.at ?? leave.date)}
                        </div>
                      </div>
                      <StatusChip status={leave.status} />
                    </div>
                    <div className="text-[11px] text-muted-foreground">
                      Wants to swap {formatDate(leave.date)} with your leave on{" "}
                      {leave.peerLeaveDate
      ...
      <Modal
        open={!!selectedAction && !confirmApproveOpen}
        onClose={() => {
          setSelectedActionId(null);
          setApprovalComment("");
        }}
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
              <div className="rounded-xl border border-border bg-muted/20 p-3">
                ...
                <div className="mt-1 font-semibold text-sm">
                  {getUserName(selectedAction.requesterId)}
                </div>
              </div>
              <div className="rounded-xl border border-border bg-muted/20 p-3">
                ...
                <div className="mt-1 font-semibold text-sm">
                  {formatDate(
                    selectedAction.history[0]?.at ?? selectedAction.date,
                  )}
                </div>
              </div>
              <div className="rounded-xl border border-border bg-muted/20 p-3">
              ...
              <div className="rounded-xl border border-border bg-muted/20 p-3">
                ...
                <div className="mt-1 font-semibold text-sm">
                  {selectedAction.peerLeaveDate
                    ? formatDate(selectedAction.peerLeaveDate)
                    : "Not provided"}
                </div>
              </div>
            </div>
            ...
            {isSameDayIncomingSwap && (
              <div className="rounded-xl border border-destructive/15 bg-destructive/5 px-4 py-3 text-xs font-semibold text-destructive">
                Same-day swaps cannot be approved. Ask the requester to select a
                different leave date.
              </div>
            )}
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                disabled={isSameDayIncomingSwap}
      ...
      <Modal
        ...
        title="Confirm Swap Approval"
      >
        <div className="space-y-5">
          <p className="text-sm text-muted-foreground">
            Confirming will send this swap request to the supervisor for final
            approval.
          </p>
          {selectedAction && (
            <div className="rounded-xl border border-border bg-muted/20 p-4 text-sm">
              ...
              <div className="text-muted-foreground mt-1">
                {formatDate(selectedAction.date)} swapped with{" "}
                {selectedAction.peerLeaveDate
                  ? formatDate(selectedAction.peerLeaveDate)
                  : "your leave"}
              </div>
          ...
          <div className="flex justify-end gap-3">
            ...
            <button
              onClick={handleApproveIncomingSwap}
              disabled={isSameDayIncomingSwap}

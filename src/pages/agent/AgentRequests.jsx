...
export default function AgentRequests() {
  ...
  const outgoing = useMemo(
    () =>
      leaves.filter(
        (l) =>
          l.requesterId === currentUser?.id &&
          (l.type === "Swap" || l.type === "Transfer"),
      ),
    [leaves, currentUser],
  );
  const incoming = useMemo(
    () =>
      leaves.filter(
        (l) => l.peerId === currentUser?.id && l.status === "PendingPeer",
      ),
    [leaves, currentUser],
  );
        ...
        {["outgoing", "incoming"].map((t) => (
          ...
          >
            ...
            {t === "incoming" && incoming.length > 0 && (
              <span className="ml-1 bg-accent text-accent-foreground text-[10px] px-2 py-0.5 rounded-full font-bold">
                {incoming.length}
              </span>
            )}
          </button>
        ...
        {items.length === 0 ? (
          <div className="py-20 text-center">
            ...
            <p className="text-[11px] text-muted-foreground/40 mt-1">
              {tab === "outgoing"
                ? "Submit a swap or transfer to see it here"
                : "Peer requests will appear here"}
            </p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-start gap-4">
                  <div className="flex items-center -space-x-2.5">
                    <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-primary/15 to-primary/5 flex items-center justify-center text-[10px] font-bold text-primary border-2 border-background z-10">
                      {getInitials(l.requesterId)}
                    </div>
                    {l.peerId && (
                      <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-info/15 to-info/5 flex items-center justify-center text-[10px] font-bold text-info border-2 border-background">
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-2.5 flex-wrap">
                      ...
                      <span className="text-[10px] bg-secondary/60 px-2.5 py-0.5 rounded-lg font-bold uppercase tracking-wider border border-border/20">
                        {l.type}
                      </span>
                    </div>
                    <p className="text-xs text-muted-foreground">
                      {tab === "outgoing"
                        ? `→ ${getUserName(l.peerId ?? "")}`
                        : `← ${getUserName(l.requesterId)}`}
                    </p>
                  </div>
                </div>
                  ...
                  {tab === "incoming" && l.status === "PendingPeer" && (
                    <div className="flex gap-2">
                      <button
                        ...
                        className="px-4 py-2 text-xs font-bold bg-success/10 text-success border border-success/20 rounded-xl hover:bg-success/18 hover:shadow-lg hover:shadow-success/10 transition-all"
                      >
                        Accept
                      </button>
                      <button
                        onClick={() => handleReject(l.id)}
                        className="px-4 py-2 text-xs font-bold bg-destructive/10 text-destructive border border-destructive/20 rounded-xl hover:bg-destructive/18 hover:shadow-lg hover:shadow-destructive/10 transition-all"
                      >
                        Reject
                      </button>
                    </div>

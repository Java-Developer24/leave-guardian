...
export default function AgentSummary() {
  ...
  const [swapLeaveId, setSwapLeaveId] = useState(null);
  ...
  const myDept = departments.find(
    (dept) => dept.id === currentUser?.departmentId,
  );
  ...
  const monthButtonLabel = visibleMonth.toLocaleDateString("en-US", {
    month: "long",
  });

  const getUserName = (id) => users.find((user) => user.id === id)?.name ?? id;
  const getBufferAlert = (date) => formatBufferAlert(date, 72, liveNow);
  const getApprovalTimer = (submittedAt) =>
    submittedAt ? getApprovalCountdown(submittedAt, 72, liveNow) : null;
  ...
  const relatedLeaves = useMemo(() => {
    return leaves.filter(
      (leave) =>
        ...
        ((leave.type === "Transfer" || leave.type === "Swap") &&
          leave.peerId === currentUser?.id),
    );
  }, [leaves, currentUser]);
  ...
  const monthLeaves = useMemo(() => {
    let result = relatedLeaves.filter((leave) =>
  ...
  const approved = monthLeaves.filter(
    (leave) => leave.status === "Approved",
  ).length;
  const pending = monthLeaves.filter((leave) =>
    ["PendingSupervisor", "PendingPeer", "Submitted"].includes(leave.status),
  ).length;
  const pendingSupervisorLeaves = relatedLeaves
    .filter(
      (leave) =>
        leave.requesterId === currentUser?.id &&
        ["PendingSupervisor", "Submitted"].includes(leave.status),
    )
    .sort((a, b) =>
      (a.history[0]?.at ?? a.date).localeCompare(b.history[0]?.at ?? b.date),
    );
  const nextActionableLeave =
    relatedLeaves
      .filter(
        (leave) =>
          leave.requesterId === currentUser?.id &&
  ...
  const canManageLeave = (leaveDate, requesterId, status) => {
    return (
      requesterId === currentUser?.id &&
      leaveDate >= todayKey &&
      ["PendingSupervisor", "PendingPeer", "Approved", "Submitted"].includes(
        status,
      )
    );
  };

  const getTransferContext = (requesterId, peerId) => {
  ...
  const getSwapContext = (requesterId, peerId) => {
    if (!peerId) return null;
  ...
  const swapSourceLeave =
    relatedLeaves.find(
      (leave) =>
        leave.id === swapLeaveId && leave.requesterId === currentUser?.id,
    ) ?? null;
  const cancelLeave =
    relatedLeaves.find(
      (leave) =>
        leave.id === cancelLeaveId && leave.requesterId === currentUser?.id,
    ) ?? null;
  const selectedSwapPeer = deptPeers.find((peer) => peer.id === swapPeer);
  const selectedSwapPeerLeaveLabel = selectedSwapPeer
    ? `${selectedSwapPeer.name}'s Leaves`
    : "Selected Agent Leaves";
  const swapSourceBufferAlert = swapSourceLeave
    ? getBufferAlert(swapSourceLeave.date)
    : null;
  const swapPeerBufferAlert = swapPeerDate
    ? getBufferAlert(swapPeerDate)
    : null;
  const isSameDaySwapSelection = Boolean(
    swapSourceLeave && swapPeerDate && swapSourceLeave.date === swapPeerDate,
  );
  ...
  return (
    <motion.div {...pageTransition}>
      <SectionHeader
        ...
        action={
          <div className="flex items-center gap-3 flex-wrap justify-end">
            ...
            <Link
              to="/agent/leave"
              className="btn-primary-gradient text-primary-foreground font-bold px-5 py-2.5 rounded-xl text-sm flex items-center gap-2"
            >
      ...
      <motion.div
        ...
        initial="initial"
        animate="animate"
        className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.5fr)_minmax(320px,0.95fr)] gap-5 mb-6"
      >
        <motion.div
          ...
          <div className="flex items-center justify-between gap-3 mb-4">
            <div>
              ...
              <div className="text-[11px] text-muted-foreground">
                Supervisor SLA countdown for your submitted leave requests.
              </div>
            </div>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <div className="text-[11px] text-muted-foreground mt-1">
                          {leave.type} leave waiting on supervisor approval
                        </div>
                      </div>
        ...
        >
          <div className="flex items-center justify-between gap-3 mb-4">
            <div>
              ...
              <div className="text-[11px] text-muted-foreground">
                Current-month leave you can still manage.
              </div>
            </div>
          ...
          {!nextActionableLeave ? (
            <div className="rounded-xl border border-border bg-muted/20 px-4 py-8 text-center text-xs text-muted-foreground">
              No current-month leave is currently available for swap or
              cancellation.
            </div>
          ) : (
            <div className="space-y-3">
              <div className="rounded-xl border border-border bg-background/80 px-4 py-3">
                <div className="text-sm font-semibold">
                  {formatDate(nextActionableLeave.date)}
                </div>
                <div className="mt-1 text-[11px] text-muted-foreground">
                  {new Date(nextActionableLeave.date).toLocaleDateString(
                    "en-US",
                    { weekday: "long" },
                  )}
                </div>
                <div className="mt-1 text-[11px] text-muted-foreground">
                  {nextActionableLeave.type} • {nextActionableLeave.status}
                </div>
              </div>
              ...
              {(() => {
                const nextApprovalTimer = [
                  "PendingSupervisor",
                  "Submitted",
                ].includes(nextActionableLeave.status)
                  ? getApprovalTimer(nextActionableLeave.history[0]?.at)
                  : null;
                if (!nextApprovalTimer || nextApprovalTimer.overdue)
                  return null;
      ...
      <div className="bg-card border border-border rounded-xl px-4 py-3 mb-4 flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          className="glass-input w-auto min-w-[140px] py-2 text-xs"
        >
        ...
        <select
          ...
          className="glass-input w-auto min-w-[120px] py-2 text-xs"
        >
          <option value="all">All Types</option>
          {["Planned", "Unplanned", "Swap"].map((type) => (
        ...
        <div className="relative ml-auto">
          ...
          <input
            ...
            placeholder="Search remarks or names..."
            className="glass-input pl-8 py-2 text-xs w-[220px]"
          />
        </div>
        <span className="text-xs text-muted-foreground">
          Showing{" "}
          <strong className="text-foreground">{monthLeaves.length}</strong>{" "}
          ...
          <table className="w-full text-sm premium-table">
            ...
            <tbody>
              {monthLeaves.map((leave, index) => {
                ...
                const canSwap =
                  canManage && !["Swap", "Transfer"].includes(leave.type);
                const transferContext =
                  leave.type === "Transfer"
                    ? getTransferContext(leave.requesterId, leave.peerId)
                    : null;
                const swapContext =
                  leave.type === "Swap"
                    ? getSwapContext(leave.requesterId, leave.peerId)
                    : null;
                const approvalTimer =
                  leave.requesterId === currentUser?.id &&
                  ["PendingSupervisor", "Submitted"].includes(leave.status)
                    ...
                    : null;
                const visibleApprovalTimer =
                  approvalTimer && !approvalTimer.overdue
                    ? approvalTimer
                    : null;
                    ...
                    <td>
                      {canManage ? (
                        <div className="flex items-center gap-2">
                          {canSwap && (
                            <button
                              onClick={() => openSwapModal(leave.id)}
                              title="Swap"
                              className="inline-flex items-center gap-1.5 rounded-lg border border-info/20 px-2.5 py-1.5 text-[11px] font-semibold text-info hover:bg-info/8 transition-colors"
                            >
                              <Repeat size={12} /> Swap
                            </button>
                          )}
                          <button
                            ...
                            title="Cancel"
                            className="inline-flex items-center gap-1.5 rounded-lg border border-destructive/15 px-2.5 py-1.5 text-[11px] font-semibold text-destructive hover:bg-destructive/8 transition-colors"
                          >
                ...
                <tr>
                  <td colSpan={7} className="p-10 text-center">
                    ...
                    <p className="text-muted-foreground text-sm">
                      No records found for {monthLabel}
                    </p>
                  </td>
                </tr>
              )}
          ...
          {monthLeaves.map((leave) => {
            ...
            const approvalTimer =
              ...
              ["PendingSupervisor", "Submitted"].includes(leave.status)
                ...
                : null;
            const visibleApprovalTimer =
              approvalTimer && !approvalTimer.overdue ? approvalTimer : null;
      ...
      <Modal
        open={!!swapSourceLeave && !swapConfirmOpen}
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
              <div className="rounded-xl border border-border bg-muted/20 p-3">
                ...
                <div className="mt-1 font-semibold text-sm">
                  {formatDate(swapSourceLeave.date)}
                </div>
              </div>
              <div className="rounded-xl border border-border bg-muted/20 p-3">
                <div className="text-muted-foreground">Leave Status</div>
                <div className="mt-1 font-semibold text-sm">
                  {swapSourceLeave.status}
                </div>
              </div>
            </div>
              ...
              <div className="space-y-3">
                ...
                {!swapPeer ? (
                  ...
                  <div className="rounded-xl border border-slate-300/60 bg-slate-200/60 px-4 py-6 text-xs text-slate-700 text-center">
                    {selectedSwapPeer?.name ?? "This peer"} has no available
                    leaves for this month.
                  </div>
                ) : (
                  <div className="space-y-3">
                    {swapPeerLeaves.map((leave) => {
                      const isSameDayOption =
                        leave.date === swapSourceLeave.date;
            ...
            {isSameDaySwapSelection && (
              <div className="rounded-xl border border-destructive/15 bg-destructive/5 px-4 py-3 text-xs font-semibold text-destructive">
                ...
                for {selectedSwapPeer?.name ?? "the selected agent"}.
              </div>
            )}
            ...
            {(swapSourceBufferAlert || swapPeerBufferAlert) && (
              <div className="rounded-xl border border-warning/20 bg-warning/10 px-4 py-3 text-xs font-semibold text-warning space-y-1">
                {swapSourceBufferAlert && (
                  <div>Your leave: {swapSourceBufferAlert}</div>
                )}
                {swapPeerBufferAlert && (
                  <div>Peer leave: {swapPeerBufferAlert}</div>
                )}
              </div>
            )}
        ...
        <div className="space-y-5">
          ...
          {swapSourceLeave && (
            <div className="rounded-xl border border-border bg-muted/20 p-4 text-sm">
              <div className="font-semibold">
                {formatDate(swapSourceLeave.date)} requested for swap
              </div>
              <div className="text-muted-foreground mt-1">
                With{" "}
                {selectedSwapPeer ? selectedSwapPeer.name : "selected peer"} on{" "}
                {swapPeerDate ? formatDate(swapPeerDate) : "their chosen leave"}
              </div>
            </div>
          )}
          {(swapSourceBufferAlert || swapPeerBufferAlert) && (
          ...
          <div className="flex justify-end gap-3">
            ...
            <button
              ...
              disabled={isSameDaySwapSelection}
              className="px-5 py-2.5 rounded-xl btn-primary-gradient text-primary-foreground text-sm font-bold disabled:opacity-40"
            >
              Confirm Request
            </button>
          </div>
      ...
      <Modal
        open={!!cancelLeave && !cancelConfirmOpen}
        onClose={() => {
          setCancelLeaveId(null);
          setCancelConfirmOpen(false);
        }}
      ...
      >
        {cancelLeave && (
          <div className="space-y-5">
            <p className="text-sm text-muted-foreground">
              This will remove the leave from the active queue. Continue to the
              final confirmation?
            </p>
            <div className="rounded-xl border border-border bg-muted/20 p-4 text-sm">
              <div className="font-semibold">
                {formatDate(cancelLeave.date)}
              </div>
              <div className="text-muted-foreground mt-1">
                {cancelLeave.type} • {cancelLeave.status}
              </div>
            </div>
            <textarea
              value={swapComment}
      ...
      >
        <div className="space-y-5">
          <p className="text-sm text-muted-foreground">
            This is the final confirmation. Are you sure you want to cancel this
            leave request?
          </p>
          {cancelLeave && (
            <div className="rounded-xl border border-border bg-muted/20 p-4 text-sm">
              ...
              <div className="text-muted-foreground mt-1">
                {cancelLeave.reason || "No remarks added"}
              </div>
            </div>
          )}
          <div className="flex justify-end gap-3">

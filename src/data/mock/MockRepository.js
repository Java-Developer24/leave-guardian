...
class MockRepository {
  ...
  leaves = [...seedLeaves];
  forecastAlerts = [...seedForecastAlerts];
  weekoffSwapRequests = [...seedWeekoffSwapRequests];
  schedule = [...seedSchedule];
  attendance = [...seedAttendance];
  holidays = [...seedHolidays];
  rules = { ...seedRules };
  leaveWindow = { ...seedLeaveWindow };
  ...
  async getLeavesByUser(userId) {
    ...
    return this.leaves.filter(
      (l) => l.requesterId === userId || l.peerId === userId,
    );
  }
  ...
  async previewLeaveSubmission(payload) {
    await delay();
  ...
  async createLeave(payload) {
    ...
    const leave = {
      ...
      id: `l${++idCounter}`,
      history: [
        { at: submittedAt, by: payload.requesterId, action: "Submitted" },
      ],
    };
  ...
  async updateLeave(id, patch) {
    ...
    if (
      ["Rejected", "Cancelled", "Approved"].includes(this.leaves[idx].status)
    ) {
      this.markForecastAlertReviewed(id);
    } else {
      this.syncForecastAlertForLeave(
        this.leaves[idx],
        this.leaves[idx].history[0]?.at ?? new Date().toISOString(),
      );
    }
    return { ...this.leaves[idx] };
  }

  async getPendingApprovals(deptId) {
    await delay();
    return this.leaves.filter(
      (l) => l.departmentId === deptId && l.status === "PendingSupervisor",
    );
  }

  async approveLeave(id, by, note) {
  ...
  async getForecastAlerts(deptId) {
    await delay();
    return deptId
      ? this.forecastAlerts
          .filter((alert) => alert.departmentId === deptId)
          .map((alert) => ({ ...alert }))
      : this.forecastAlerts.map((alert) => ({ ...alert }));
  }

  async getWeekoffSwapRequests(deptId) {
    await delay();
    return deptId
      ? this.weekoffSwapRequests
          .filter((request) => request.departmentId === deptId)
          .map((request) => ({ ...request, history: [...request.history] }))
      : this.weekoffSwapRequests.map((request) => ({
          ...request,
          history: [...request.history],
        }));
  }

  async createWeekoffSwapRequest(payload) {
  ...
  async approveWeekoffSwapRequest(id, by, note) {
  ...
  async rejectWeekoffSwapRequest(id, by, note) {
    const request = this.weekoffSwapRequests.find((item) => item.id === id);
    if (!request) throw new Error("Week-off swap request not found");
  ...
  async getSchedule(_deptId) {
    await delay();
    return [...this.schedule];
  }
  async uploadSchedule(rows) {
    await delay();
    this.schedule = rows;
  }
  async getAttendance() {
  ...
  async uploadAttendance(rows) {
    await delay();
    this.attendance = [...this.attendance, ...rows];
  }
  async getHolidays() {
    await delay();
  ...
  async getLeaveWindow() {
    await delay();
    return { ...this.leaveWindow };
  }
  async updateLeaveWindow(patch) {
    await delay();
    this.leaveWindow = { ...this.leaveWindow, ...patch };
  }

  syncForecastAlertForLeave(leave, createdAt) {
    if (
      !["Planned", "Unplanned"].includes(leave.type) ||
      !["PendingSupervisor", "Submitted", "Approved"].includes(leave.status)
    ) {
      return;
    }
    ...
    const existingIndex = this.forecastAlerts.findIndex(
      (alert) => alert.leaveId === leave.id,
    );
    ...
    const alert = buildForecastAlert({
      alertId:
        existingIndex >= 0
          ? this.forecastAlerts[existingIndex].id
          : `fa${++alertCounter}`,
      leave,
      preview,
      createdAt,
  ...
  applyWeekoffSwapToSchedule(request) {
    ...
    if (mode === "MonthSwap") {
      ...
      const sourceMonthRows = this.schedule.filter(
        (row) =>
          row.userId === request.sourceGuideId &&
          row.date.startsWith(request.monthKey),
      );
      const peerMonthRows = this.schedule.filter(
        (row) =>
          row.userId === request.peerGuideId &&
          row.date.startsWith(request.monthKey),
      );

      sourceMonthRows.forEach((sourceRow) => {
        const peerRow = peerMonthRows.find(
          (row) => row.date === sourceRow.date,
        );
        if (!peerRow) return;
    ...
    const sourceOwnDay = this.schedule.find(
      (row) =>
        row.userId === request.sourceGuideId && row.date === request.sourceDate,
    );
    const sourceTargetDay = this.schedule.find(
      (row) =>
        row.userId === request.sourceGuideId && row.date === request.peerDate,
    );
    ...
    const peerOwnDay = this.schedule.find(
      (row) =>
        row.userId === request.peerGuideId && row.date === request.peerDate,
    );
    const peerSourceDay = this.schedule.find(
      (row) =>
        row.userId === request.peerGuideId && row.date === request.sourceDate,
    );
